...
function open_ficha(id) {
	...
	$.post("../ajax/ficha2_sanitaria_alimentacion.php?op=mostrar", { id: id }, function (data, status) {
		...
		//AGREGAMOS estos datos desde el array para que muestren la data 
		...
		$("#iiii_8").val(data[0].iiii_8);
		$("#iiii_9").val(data[0].iiii_9);
		$("#iiii_10").val(data[0].iiii_10);
	...
	table = $('#tbllistado').DataTable(
		{
			...
			"sAjaxSource": "../ajax/ficha2_sanitaria_alimentacion.php?op=listar&id_nivel=" + $("#s_id_nivel").val() + "&id_usuario=" + $("#s_id_usuario").val() + "&id_local=" + $("#s_id_local").val(), // Load Data
			...
			columns: [
				...
				{ aTargets: 'f.id' },
				{ aTargets: 'f.fecha' },
				{ aTargets: 'f.responsable_mercado' },
				{ aTargets: 'f.razon_s' },
	...
	bootbox.confirm({
		...
		callback: function (result) {
			...
			if (result) {
				...
				$.ajax({
					url: "../ajax/ficha2_sanitaria_alimentacion.php?op=save",
					type: "POST",
					data: parametros,
					//contentType: false,
					//processData: false,
...
function desactivar(id) {
	bootbox.confirm({
		...
		callback: function (result) {
			if (result) {
				$.post("../ajax/ficha2_sanitaria_alimentacion.php?op=desactivar", { id_ficha: id }, function (e) {
					bootbox.alert(e);
					table.ajax.reload();
				});
			}
...
function activar(id) {
	...
	bootbox.confirm({
		...
		callback: function (result) {
			if (result) {
				$.post("../ajax/ficha2_sanitaria_alimentacion.php?op=activar", { id_ficha: id }, function (e) {
					bootbox.alert(e);
					table.ajax.reload();
				});
			}
...
// Agregamos funcion select 2 para las vistas de las fichas 
