...
function listar()
{
	function format ( rowData ) {
		...
		.text( 'Cargando...' );
		$.ajax( {
			url: '../ajax/catalogo.php?op=tableLocales',
			data:  parametros,
			dataType: 'html',
			success: function ( json ) {
				div
	...
	table=$('#tbllistado').DataTable(
	{
		...
		"buttons": [
      {
	    ...
	    /*buttons: [
      	...
      	"sAjaxSource": "../ajax/catalogo.php?op=listar", // Load Data
		/*"ajax":
				{
		...
		"columnDefs": [
		{ "width":"2%", "orderable": false,	"targets": 0,	"searchable": false },
		...
		columns: [
		{
			...
			{ aTargets: 'i.nombre' },
			{ aTargets: 'i.id' },
			{ aTargets: 'g.descripcion' },			
			{ aTargets: 'r.descripcion' },	
...
function guardaryeditar()
{
	//
	 var msj='';
	if ($("#id_categoria").val()==''){
		var msj="Selecciona una categoria...!";	
	}else if($("#nombre").val()==''){
		var msj="Ingresa una Descripción..!";
	}
		...
		callback: function (result) {
			...
			if (result){				
				...
				var parametros = {
					...
					"id_tipo_rotacion":$('#id_tipo_rotacion').val(),
					"id_tipo_precio":$('#id_tipo_precio').val()				
					};
...
function mostrar_item(id_catalogo)
{
	$.post("../ajax/catalogo.php?op=mostrar",{id_catalogo : id_catalogo}, function(data, status)
	{
		data = JSON.parse(data);
		$("#id_catalogo").val(id_catalogo);
		$("#nombre").val(data.nombre);
		
		$("#id_categoria").val(data.id_grupo);
		$("#id_tipo_rotacion").val(data.id_tipo_rotacion);
...
function desactivar(id_item)
{
	bootbox.confirm("¿Está Seguro de desactivar el registro?", function(result){
		if(result)
        {
        	$.post("../ajax/catalogo.php?op=desactivar", {id_catalogo : id_item}, function(e){
        		bootbox.alert(e);
	            table.ajax.reload();
        	});
        }
