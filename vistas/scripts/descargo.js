...
function listar()
{
	function format ( rowData ) {
		...
		.text( 'Cargando...' );
		$.ajax( {
			url: '../ajax/descargo.php?op=detalleDescargo',
			data:  parametros,
			dataType: 'html',
			success: function ( json ) {
				div
...

	{
		...
		"buttons": [
      {
	    ...
	    /*buttons: [
      	...
      	"sAjaxSource": "../ajax/descargo.php?op=listar", // Load Data
		/*"ajax":
				{
...
function mostrar_descargo(id_requerimiento,id_nivel_usuario)
{
	//alert(id_item);
	editItem=false;
	newItem=false;
	$.post("../ajax/descargo.php?op=mostrar",{id_requerimiento : id_requerimiento}, function(data, status)
	{
		data = JSON.parse(data);

		//console.log(data);
		if (data.id_usuario_crea!=$("#id_usuario_crea").val() ){
		...
		//$('#razon_social').append("<option value='"+data.id_proveedor+"' selected='selected'>"+data.razon_social+"</option>");
 		...
 		$.each(data.detalle, function( i, item ){
		    ...
		    cell3.innerHTML = item.marca;
		    cell3.id="td_marca";

			cell4.innerHTML = item.unidad_medida_ingreso;
...
function desactivar(id_requerimiento)
{
	bootbox.confirm("¿Está Seguro de anular el registro?", function(result){
		if(result)
        {
        	$.post("../ajax/descargo.php?op=desactivar",
