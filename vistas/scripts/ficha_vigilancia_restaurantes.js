...
function open_ficha(id) {
	...
	$.post("../ajax/ficha_vigilancia_restaurantes.php?op=mostrar", { id: id }, function (data, status) {
		...
		$("#representante_legal").val(dat.representante_legal)
		$("#licencia").val(dat.numero_licencia)
		$("#responsable").val(dat.responsable)

		$("#cargo").val(dat.cargo)
		$("#dias_actividad").val(dat.numero_dias_actividad)
		$("#horario_atencion").val(dat.horario_atencion)
		$("#nro_hombres").val(dat.numero_hombres)
		$("#nro_mujeres").val(dat.numero_mujeres)

		$("#sector").val(dat.sector)
		//$("#id_localidad").val(dat.id_localidad)
		getLocalidades(dat.id_localidad);
		$("#long_este").val(dat.long_este)
		$("#long_norte").val(dat.long_norte)
		...
		for (let x of keys) {
			...
			if (l == 'i') {
				...
				if (x.search('obs') > 0) {
					...
					if (dat[x] == 't') {
						sum++;
						$('input[type=checkbox][name=' + x + ']').bootstrapToggle('on');

					} else {
						$('input[type=checkbox][name=' + x + ']').bootstrapToggle('off');
					}
...
function getLocalidades(id_default) {
	...
	$.ajax({
		url: '../ajax/tabla.php?op=cboLocalidades',
		data: parametros,
		method: "get",
		dataType: 'html',
		success: function (json) {
			$("#id_localidad").html(json);
...
function valorant(chk) {
	nam = chk.getAttribute("name");
	if ($('[name=' + nam + ']').is(':checked')) {
		cntValoracion++;
	} else {
		if (cntValoracion > 0) {
			cntValoracion--;
...
function printResultados() {
	$("#div_valoracion").html("Puntaje Total: " + cntValoracion.toString());
	$("#cnt_total").val(cntValoracion.toString());
	if (cntValoracion >= 0 && cntValoracion <= 26) {
		$("#div_result").html("No Aceptable");
		$("#div_result").removeClass('green orange').addClass('red');
		$("#id_resultado").val('1');

	} else if (cntValoracion >= 27 && cntValoracion <= 39) {
		$("#div_result").html("Regular");
		$("#div_result").removeClass('green red').addClass('orange');
		$("#id_resultado").val('2');
	} else {
		$("#div_result").html("Aceptable");
		$("#div_result").removeClass('green orange').addClass('green');
		$("#id_resultado").val('3');
	}
}

function resaltar(chk) {

	if ($('[name=' + chk + ']').is(':checked')) {
		//alert("Si");
		$('[name=' + chk + ']').parent('.tr_primary').removeClass("danger");
		//$('[name="i_1"]').parent('.x').addClass("success");

	} else {
		//alert("No");
		//$('[name="i_1"]').parent('.x').removeClass("success");
		$('[name=' + chk + ']').parent('.tr_primary').addClass("danger");
	}
}
	...
	table = $('#tbllistado').DataTable(
		{
			...
			"sAjaxSource": "../ajax/ficha_vigilancia_restaurantes.php?op=listar&id_nivel=" + $("#s_id_nivel").val() + "&id_usuario=" + $("#s_id_usuario").val() + "&id_local=" + $("#s_id_local").val(), // Load Data
			...
			columns: [
				...
				{ aTargets: 'e.nombre' },
				{ aTargets: 'e.ruc' },
				{ aTargets: 'f.responsable' },
				{ aTargets: 'uc.login' },
				{ aTargets: 'f.fecha_creacion' },
				{
					aTargets: 'res.descripcion',
...
function guardaryeditar() {
	...
	if ($("#ruc").val() == '') {
		...
		var msj = "Seleccione sector";
	} else if ($("#id_localidad").val() == '') {
		var msj = "Seleccione localidad";
	}
	if (msj) {
	...
	bootbox.confirm({
		...
		callback: function (result) {
		  ...
		  "id_usuario": $("#s_id_usuario").val()
				...
				$.ajax({
					url: "../ajax/ficha_vigilancia_restaurantes.php?op=save",
					type: "POST",
					//data: parametros,
					data: datastring,
					//contentType: false,
...
function desactivar(id) {
	bootbox.confirm("¿Está Seguro de desactivar el registro?", function (result) {
		if (result) {
			$.post("../ajax/ficha_vigilancia_restaurantes.php?op=desactivar", { id: id }, function (e) {
				bootbox.alert(e);
				table.ajax.reload();
			});
		}
...
function activar(id) {
	bootbox.confirm("¿Está Seguro de activar el registro?", function (result) {
		if (result) {
			$.post("../ajax/ficha_vigilancia_restaurantes.php?op=activar", { id: id }, function (e) {
				bootbox.alert(e);
				table.ajax.reload();
			});
		}
