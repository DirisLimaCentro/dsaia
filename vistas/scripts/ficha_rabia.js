...
function open_ficha(id)
{
  ...
  $.post("../ajax/ficha_rabia.php?op=mostrar",{id : id}, function(data, status)
  {
    ...
    $("#cnt_vacuna_sextuple").val(data[0].cnt_vacuna_sextuple);
    $("#cnt_vacuna_ant_ins").val(data[0].cnt_vacuna_ant_ins);
    $("#cnt_vacuna_ant_lab").val(data[0].cnt_vacuna_ant_lab);
    $("#nombre_lab").val(data[0].nombre_lab);
    $("#otros_animales").val(data[0].otros_animales)
...
function limpiar()
{
  ...
  $('#vacuna_antitetanica').bootstrapToggle('off');
  $('#mordedura_ultimo').bootstrapToggle('off');
  $('#acudio_es').bootstrapToggle('off');
  $('#conoce_triada').bootstrapToggle('off');
  $('#facilita_informacion').bootstrapToggle('off');
  $('#permite_observar').bootstrapToggle('off');
  $('#verifica_temperatura').bootstrapToggle('off');
  $('#verifica_equipos').bootstrapToggle('off');

  $('#apoya_vigilancia').bootstrapToggle('off');
  $('#guia_sanitaria').bootstrapToggle('off');
  $('#autoriza_correo').bootstrapToggle('off');
	...
	"buttons": [
      {
	    ...
	    /*buttons: [
      	...
      	"sAjaxSource": "../ajax/ficha_rabia.php?op=listar&id_nivel="+$("#s_id_nivel").val()+"&id_usuario="+$("#s_id_usuario").val()+"&id_local="+$("#s_id_local").val(), // Load Data
...
function guardaryeditar()
{
    ...
    var msj="Ingrese nombres del responsable";
  }
  if (msj){
  ...
  if ($('#vacuna_antitetanica').is(':checked')) var vacuna_antitetanica=true; else var vacuna_antitetanica=false;
  if ($('#mordedura_ultimo').is(':checked')) var mordedura_ultimo=true; else var mordedura_ultimo=false;
  if ($('#acudio_es').is(':checked')) var acudio_es=true; else var acudio_es=false;
  if ($('#conoce_triada').is(':checked')) var conoce_triada=true; else var conoce_triada=false;
  if ($('#permite_observar').is(':checked')) var permite_observar=true; else var permite_observar=false;
  if ($('#facilita_informacion').is(':checked')) var facilita_informacion=true; else var facilita_informacion=false;
  if ($('#verifica_temperatura').is(':checked')) var verifica_temperatura=true; else var verifica_temperatura=false;
  if ($('#verifica_equipos').is(':checked')) var verifica_equipos=true; else var verifica_equipos=false;
  if ($('#apoya_vigilancia').is(':checked')) var apoya_vigilancia=true; else var apoya_vigilancia=false;
  if ($('#guia_sanitaria').is(':checked')) var guia_sanitaria=true; else var guia_sanitaria=false;
  if ($('#autoriza_correo').is(':checked')) var autoriza_correo=true; else var autoriza_correo=false;
	...
	bootbox.confirm({
        ...
        strEspecies=$("#id_especie").val();
        if (strEspecies){
          strEspecies=strEspecies.toString();
        }else{
          strEspecies='';
        }  
				var parametros = {
          ...
          "vacuna_antirabica": vacuna_antirabica,
          "vacuna_antitetanica": vacuna_antitetanica,
          "mordedura_ultimo": mordedura_ultimo,
          "acudio_es": acudio_es,
          "conoce_triada": conoce_triada,
          "facilita_informacion": facilita_informacion,
          "permite_observar": permite_observar,
          "verifica_temperatura": verifica_temperatura,
          "verifica_equipos": verifica_equipos,
          "apoya_vigilancia": apoya_vigilancia,
          "guia_sanitaria": guia_sanitaria,
          "autoriza_correo": autoriza_correo,

          "cnt_vacuna_sextuple": $('#cnt_vacuna_sextuple').val(),
          "cnt_vacuna_ant_ins": $('#cnt_vacuna_ant_ins').val(),
          "cnt_vacuna_ant_lab": $('#cnt_vacuna_ant_lab').val(),

          "nombre_lab":$('#nombre_lab').val(),
          "otros_animales":$('#otros_animales').val(),
          "id_local": $("#s_id_local").val(),
          "id_usuario": $("#s_id_usuario").val(),
          "id_especies": strEspecies,
          "fecha": $("#fecha_encuesta").val()
				...
				$.ajax({
					url: "../ajax/ficha_rabia.php?op=save",
					type: "POST",
					data: parametros,
					//contentType: false,
					//processData: false,
