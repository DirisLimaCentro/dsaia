...
function open_ficha(id) {
	...
	$.post("../ajax/ficha1_sanitaria_alimentacion.php?op=mostrar", { id: id }, function (data, status) {
	...
	table = $('#tbllistado').DataTable(
		{
			...
			"sAjaxSource": "../ajax/ficha1_sanitaria_alimentacion.php?op=listar&id_nivel=" + $("#s_id_nivel").val() + "&id_usuario=" + $("#s_id_usuario").val() + "&id_local=" + $("#s_id_local").val(), // Load Data
	...
	bootbox.confirm({
		...
		callback: function (result) {
			...
			if (result) {
				...
				$.ajax({
					url: "../ajax/ficha1_sanitaria_alimentacion.php?op=save",
					type: "POST",
					data: parametros,
					//contentType: false,
					//processData: false,
...
function desactivar(id) {
	bootbox.confirm({
		...
		callback: function (result) {
			if (result) {
				$.post("../ajax/ficha1_sanitaria_alimentacion.php?op=desactivar", { id_ficha: id }, function (e) {
					bootbox.alert(e);
					table.ajax.reload();
				});
			}
...
function activar(id) {
	...
	bootbox.confirm({
		...
		callback: function (result) {
			if (result) {
				$.post("../ajax/ficha1_sanitaria_alimentacion.php?op=activar", { id_ficha: id }, function (e) {
					bootbox.alert(e);
					table.ajax.reload();
				});
			}
