...
function open_ficha(id)
{
  ...
  $.post("../ajax/ficha_sanitaria_alimentacion.php?op=mostrar",{id : id}, function(data, status)
  {
    ...
    $('#id_medico_jefe').append("<option value='"+data[0].id_medico_jefe+"' selected='selected'>"+data[0].medico_jefe+"</option>");
    $("#id_medico_jefe").trigger('change');


    $("#responsable_modulo_venta").val(data[0].responsable_modulo_venta);
    $("#numero_carnet").val(data[0].numero_carnet);
    ...
    $("#ii_6").val(data[0].ii_6);
    $("#ii_7").val(data[0].ii_7);
    ...
    $("#cnt_extintores").val(data[0].cnt_extintores_vig);
    $("#cnt_senalizadores").val(data[0].cnt_senales);
    $("#cnt_botiquin").val(data[0].cnt_botiquines);
    $("#cnt_sep").val(data[0].cnt_sep);
...
function limpiar()
{
  ...
  $("#numero_carnet").val("");
  $("#numero_manipuladores").val("");  
  $("#tipo_alimentos").val("");
  $("#responsable_modulo_venta").val("");

     
  $('#id_medico_jefe').val('').trigger('change.select2');
	...
	"buttons": [
      {
	    ...
	    /*buttons: [
      	...
      	"sAjaxSource": "../ajax/ficha_sanitaria_alimentacion.php?op=listar&id_nivel="+$("#s_id_nivel").val()+"&id_usuario="+$("#s_id_usuario").val()+"&id_local="+$("#s_id_local").val(), // Load Data
    ...
    initComplete: function () {
      ...
      table.columns().every( function () {
		...
		columns: [
			...
			{ aTargets: 'f.id' },
			{ aTargets: 'f.fecha' },
			{ aTargets: 'f.responsable_modulo_venta' },
			{ aTargets: 'f.numero_carnet' },
...
function guardaryeditar()
{
	...
	}else if ($("#id_medico_jefe").val()==''){
		var msj="Ingrese medico jefe del establecimiento";
	}else if ($("#responsable_modulo_venta").val()==''){
    var msj="Ingrese responsable del modulo de venta";
  }else if ($("#numero_carnet").val()==''){
	...
	bootbox.confirm({
		...
		callback: function (result) {
          ...
          "id_usuario": $("#s_id_usuario").val(),
					...
					"id_medico_jefe":$("#id_medico_jefe").val(),          
          "responsable_modulo_venta":$('#responsable_modulo_venta').val().toUpperCase(),
					"numero_carnet":$('#numero_carnet').val().toUpperCase(),          
          ...
          "cnt_extintores":$('#cnt_extintores').val(),
          "cnt_senalizadores":$('#cnt_senalizadores').val(),
          "cnt_botiquin":$('#cnt_botiquin').val(),
          "cnt_sep":$('#cnt_sep').val()
				...
				$.ajax({
					url: "../ajax/ficha_sanitaria_alimentacion.php?op=save",
					type: "POST",
					data: parametros,
					//contentType: false,
					//processData: false,
