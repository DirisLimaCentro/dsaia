...
function renderData(){
  ...
  var parametros = {"desde":$("#fecha_desde").val(), "hasta":$("#fecha_hasta").val(),"tipo":tipo, "id_establecimiento": id_establecimiento};
  $.ajax( {
...
// Generar 20 colores aleatorios y agregarlos al array
for (var i = 0; i < 30; i++) {
    colors.push(randomHexColor());
}


  $.get("../ajax/ficha_vivienda.php?op=getCoordenadasFumigacion",{desde :$("#fecha_desde").val(),hasta: $("#fecha_hasta").val(), id_establecimiento: id_establecimiento}, function(data, status)
  {
      ...
      // Inicializa un array vacío para almacenar las coordenadas convertidas
      routes  = [];
      ...
      $.each(data.detalle, function( i, item ){
        ...
        var coordArray = [];
        //lat=jsonArray[0]['latitud'];
        //long=jsonArray[0]['longitud'];
        //coordenadasArray[0].push({lat: lat, lng: long});        
        $.each(jsonArray,function(j, it) {
            lat=parseFloat(it.latitud);
            lng=parseFloat(it.longitud);
            coordArray.push({location: {lat: lat, lng: lng} });
        });

        arrayOri={lat:parseFloat(jsonArray[0]['latitud']) ,  lng: parseFloat(jsonArray[0]['longitud'])};
        arrayDes={lat:parseFloat(jsonArray[jsonArray.length - 1]['latitud']) ,  lng: parseFloat(jsonArray[jsonArray.length - 1]['longitud'])};

        //routes.push({origin:arrayOri, destination:arrayDes, waypoints:coordArray, color: colors[n]   });
        routes.push({waypoints:coordArray, color: colors[n]   });
        n++;
...
function renderRoutes(){
    ...
    routes.forEach(function(route) {
        var polyline = new google.maps.Polyline({
            path: route.waypoints.map(wp => wp.location),
            geodesic: true,
...
function renderMapaRut(){
    ...
    var map = new google.maps.Map(document.getElementById('map'), {
        ...
        var directionsRenderer = new google.maps.DirectionsRenderer({
            map: map,
            polylineOptions: {
                    strokeColor: '#58ACFA' // Cambia el color de la ruta aquí
            }
        });

        var districtData = locationsb.map(item=>{
            //return {  lat: item[0], lng: item[1], weight: 8 }
            return {  lat: item[0], lng: item[1], weight: 8, color:'red' }
        })
        ...
        districtData.forEach(function (coordenada) {
             ...
             if (i==0 ){
                start=new google.maps.LatLng(coordenada.lat, coordenada.lng);
             }else if ((i+1)==districtData.length){
                end=new google.maps.LatLng(coordenada.lat, coordenada.lng);                
             }else{
                if (i<23 ){
                    waypoints.push({ location: new google.maps.LatLng(coordenada.lat, coordenada.lng) })                
...
function initAutocomplete() {
    ...
    autocomplete = new google.maps.places.Autocomplete(
        /** @type {!HTMLInputElement} */(document.getElementById('direccion_ruta')),
        {
            types: ['geocode'],
...
function fillInAddress() {
    ...
    $("#latitud_ruta").val(lat);
    $("#longitud_ruta").val(lng);
...
function save_item_f(){
      ...
      msg='';
      if ($("#lugar_ruta").val()==''){
          bootbox.alert("Ingrese lugar");
          return false;  
      }
      if ($("#direccion_ruta").val()==''){
         bootbox.alert("Ingrese latitud ruta");
         return false;  
      }
          ...
          if (result){
              ...
              $.ajax({
                ...
                data : {
                  ...
                  lugar: $("#lugar_ruta").val().toUpperCase(),
                  direccion_ruta: $("#direccion_ruta").val().toUpperCase(),
                  latitud_ruta: $("#latitud_ruta").val(),
                  longitud_ruta: $("#longitud_ruta").val(),
                  orden: $("#orden_ruta").val(),
...
function update_child(row){
	...
	$.ajax( {
		url: '../ajax/ficha_fumigacion.php?op=detalleFicha&id_usuario='+$("#s_id_usuario").val(),
		data:  parametros,
		dataType: 'html',
		success: function ( json ) {
			$("#row_"+row).html( json );
...
function listar()
{
...

	{
	    ...
	    /*buttons: [
      	...
      	"sAjaxSource": "../ajax/ficha_fumigacion.php?op=listar&id_nivel="+$("#s_id_nivel").val()+"&id_usuario="+$("#s_id_usuario").val()+"&id_local="+$("#s_id_local").val(), // Load Data
		/*"ajax":
				{
		...
		columns: [
			...
			//{ aTargets: null },
			{ aTargets: 'f.id' },			
			{ aTargets: 'f.fecha_programada' },
			{ aTargets: 'f.hora_salida' },		
			{ aTargets: 'l.nombre' },
			{ aTargets: 'f.hora_llegada' },
			{ aTargets: 'f.area' },
			{ aTargets: 'f.participa' },
			{ aTargets: 'f.organiza' },
            { aTargets: 'f.actividad' },
