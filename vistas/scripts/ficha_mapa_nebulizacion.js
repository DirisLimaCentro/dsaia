...
function renderData(){
  ...
  $.ajax( {
    url: '../ajax/ficha_vivienda.php?op=dataNebulizacion',
    data:  parametros,
    dataType: 'html',
    async: false,
...
function buscarPorNombre(arr, nombreABuscar) {
  for (var i = 0; i < arr.length; i++) {
    if (arr[i].etiqueta === nombreABuscar) {
      return arr[i];
    }
  }
  return null; // Retornar null si el país no se encuentra en el arreglo
}
...
function getData(){
  ...
  $.get("../ajax/ficha_vivienda.php?op=getCoordenadasNebulizacion",{desde :$("#fecha_desde").val(),hasta :$("#fecha_hasta").val(),id_establecimiento: id_establecimiento}, function(data, status)
  {
      ...
      $.each(data.detalle, function( i, item ){
        ...
        if (buscarPorNombre(marcadores, item.establecimiento)==null){
            marcadores.push({ lat: parseFloat(item.latitud_est), lng: parseFloat(item.longitud_est), etiqueta: item.establecimiento, id: item.id_local })
        }    
...
function renderMapaRut(){
    ...
    var map = new google.maps.Map(document.getElementById('map'), {
        ...
        marcadores.forEach(function(marcador) {
            ...
            //contenido='<div><img src="images/nebulizacion/'+marcador.id+'.jpg" width="200px"></div>'
            var infowindow = new google.maps.InfoWindow({
                content: '<div><h2>' + marcador.etiqueta + '</h2><img src="images/nebulizacion/' + marcador.id + '.jpg" width="400px"></div>'
            });
