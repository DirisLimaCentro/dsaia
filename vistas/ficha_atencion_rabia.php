...
fieldset.scheduler-border {
    ...
    padding: 0 1px 0 1.4em !important;
        ...
        <div class="row">
          <div class="col-md-12 col-sm-12 col-xs-12">
            ...
            <div class="x_panel">
              ...
              <div class="x_title">
                <h2>Ficha Atencion a personas expuestas al virus rábico <small></small></h2>
              ...
              <? require 'partials/modal_ficha_atencion_rabia.php'; ?>
...
<!-- Modal nueva Localidad--->
  ...
  <script type="text/javascript" src="scripts/ficha_atencion_rabia.js?rnd=<?php echo rand(); ?>"></script>
...
function ver(){
  ...
  $('#modalNew').modal('show')
    ...
    $(function () {
      ...
      $.ajax( {
        url: '../ajax/tabla.php?op=cbotabla&id_tabla=69',
        //data:  parametros,
        dataType: 'html',
    ...
    $(".select2").select2();
     ...
     $.getJSON('../ajax/tabla.php?op=list&id_tabla=17', function(data) {
      ...
      $.getJSON('../ajax/tabla.php?op=list&id_tabla=41', function(data) {
      var template = $('#tpl_situacion_animal').html();
      var html = Mustache.to_html(template, data);
      $('#id_estado_actual_animal').html(html);
    });
      ...
      $.getJSON('../ajax/tabla.php?op=list&id_tabla=1', function(data) {
        ...
        var html = Mustache.to_html(template, data);
        $('#tipo_doc').html(html);
        $('#tipo_doc_acompanante').html(html);
      });

      /*$.getJSON('../ajax/tabla.php?op=list&id_tabla=1', function(data) {
        var template = $('#tpl_tipo_doc_acompanante').html();
        var html = Mustache.to_html(template, data);
        $('#tipo_doc_acompanante').html(html);
      });*/
      ...
      $.getJSON('../ajax/tabla.php?op=list&id_tabla=54', function(data) {
      ...
      $.getJSON('../ajax/tabla.php?op=list&id_tabla=43', function(data) {
        var template = $('#tpl_localizacion').html();
        var html = Mustache.to_html(template, data);
        $('#id_localizacion').html(html);
        $('#id_localizacion').selectpicker();
      });

      $.getJSON('../ajax/tabla.php?op=list&id_tabla=67', function(data) {
        var template = $('#tpl_grado_instruccion').html();
        var html = Mustache.to_html(template, data);
        $('#id_grado_instruccion').html(html);        
      });

      $.getJSON('../ajax/tabla.php?op=list&id_tabla=44', function(data) {
        var template = $('#tpl_tipo_proteccion').html();
        var html = Mustache.to_html(template, data);
        $('#id_tipo_proteccion').html(html);        
      });

       $.getJSON('../ajax/tabla.php?op=list&id_tabla=45', function(data) {
        var template = $('#tpl_numero_lesiones').html();
        var html = Mustache.to_html(template, data);
        $('#id_numero_lesiones').html(html);        
      });
       
      $.getJSON('../ajax/tabla.php?op=list&id_tabla=47', function(data) {
        var template = $('#tpl_estado_lesion').html();
        var html = Mustache.to_html(template, data);
        $('#id_estado_herida').html(html);        
      });
          $.getJSON('../ajax/tabla.php?op=list&id_tabla=48', function(data) {
        var template = $('#tpl_atencion_herida').html();
        var html = Mustache.to_html(template, data);
        $('#id_atencion_herida').html(html);        
      });
      $.getJSON('../ajax/tabla.php?op=list&id_tabla=49', function(data) {
        var template = $('#tpl_lugar_suceso').html();
        var html = Mustache.to_html(template, data);
        $('#id_lugar_suceso').html(html);        
      });

       $.getJSON('../ajax/tabla.php?op=list&id_tabla=51', function(data) {
        var template = $('#tpl_alergico').html();
        var html = Mustache.to_html(template, data);
        $('#id_alergico').html(html);        
      });

      $.getJSON('../ajax/tabla.php?op=list&id_tabla=53', function(data) {
        var template = $('#tpl_tipo_propietario').html();
        var html = Mustache.to_html(template, data);
        $('#id_tipo_propietario').html(html);        
      });

      $.getJSON('../ajax/tabla.php?op=list&id_tabla=68', function(data) {
        var template = $('#tpl_condicion_animal').html();
        var html = Mustache.to_html(template, data);
        $('#id_situacion').html(html);        
      });

      $.getJSON('../ajax/tabla.php?op=list&id_tabla=70', function(data) {
        var template = $('#tpl_tipo_exp').html();
        var html = Mustache.to_html(template, data);
        $('#tipo_exposicion').html(html);        
      });
...
<script id="tpl_tipo_doc_acompanante" type="text/template"><option value='' selected>--Seleccione--</option>
  {{#tablas}}<option value='{{id}}'>{{nombre}}</option>{{/tablas}}
</script>

<script id="tpl_grado_instruccion" type="text/template"><option value='' selected>--Seleccione--</option>
  {{#tablas}}<option value='{{id}}'>{{nombre}}</option>{{/tablas}}
</script>


<script id="tpl_tipo_proteccion" type="text/template"><option value='' selected>--Seleccione--</option>
  {{#tablas}}<option value='{{id}}'>{{nombre}}</option>{{/tablas}}
</script>

<script id="tpl_numero_lesiones" type="text/template"><option value='' selected>--Seleccione--</option>
  {{#tablas}}<option value='{{id}}'>{{nombre}}</option>{{/tablas}}
</script>

<script id="tpl_tipo_lesion" type="text/template"><option value='' selected>--Seleccione--</option>
  {{#tablas}}<option value='{{id}}'>{{nombre}}</option>{{/tablas}}
</script>

<script id="tpl_estado_lesion" type="text/template"><option value='' selected>--Seleccione--</option>
  {{#tablas}}<option value='{{id}}'>{{nombre}}</option>{{/tablas}}
</script>

<script id="tpl_atencion_herida" type="text/template"><option value='' selected>--Seleccione--</option>
  {{#tablas}}<option value='{{id}}'>{{nombre}}</option>{{/tablas}}
</script>

<script id="tpl_lugar_suceso" type="text/template"><option value='' selected>--Seleccione--</option>
  {{#tablas}}<option value='{{id}}'>{{nombre}}</option>{{/tablas}}
</script>


<script id="tpl_especies" type="text/template"><option value='' selected>--Seleccione--</option>
  {{#tablas}}<option value='{{id}}'>{{nombre}}</option>{{/tablas}}
</script>

<script id="tpl_localizacion" type="text/template">
  {{#tablas}}<option value='{{id}}'>{{nombre}}</option>{{/tablas}}
</script>

<script id="tpl_situacion_animal" type="text/template"><option value='' selected>--Seleccione--</option>
  {{#tablas}}<option value='{{id}}'>{{nombre}}</option>{{/tablas}}
</script>

<script id="tpl_alergico" type="text/template"><option value='' selected>--Seleccione--</option>
  {{#tablas}}<option value='{{id}}'>{{nombre}}</option>{{/tablas}}
</script>

<script id="tpl_tipo_propietario" type="text/template"><option value='' selected>--Seleccione--</option>
  {{#tablas}}<option value='{{id}}'>{{nombre}}</option>{{/tablas}}
</script>

<script id="tpl_condicion_animal" type="text/template"><option value='' selected>--Seleccione--</option>
  {{#tablas}}<option value='{{id}}'>{{nombre}}</option>{{/tablas}}
</script>

<script id="tpl_tipo_exp" type="text/template"><option value='' selected>--Seleccione--</option>
  {{#tablas}}<option value='{{id}}'>{{nombre}}</option>{{/tablas}}
</script>
  <?php 
ob_end_flush();
