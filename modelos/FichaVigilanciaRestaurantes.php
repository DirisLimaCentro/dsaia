...
Class Ficha
{
	...
	public function save(
		...
		$sql="SELECT sp_crud_ficha_restaurante(
	...
	{
		$sql="UPDATE ficha_vig_restaurantes SET estado=false WHERE id='$id'";
		return ejecutarConsulta($sql);
	}
	...
	{
		$sql="UPDATE ficha_vig_restaurantes SET estado=true WHERE id='$id'";
		return ejecutarConsulta($sql);
	}
	...
	public function contar($sWhere){
		...
		inner join entidad e on e.id=f.id_entidad
		inner join usuarios uc on uc.id=f.id_usuario_crea
		inner join tablas res on res.id_tipo=f.id_resultado and res.id_tabla='85'
		".$sWhere;
		return ejecutarConsultaSimpleFila($sql);
	}
	...
	public function listar($sWhere,$sOrder,$sLimit)
	{
		...
		f.responsable as \"f.responsable\",
		uc.login as \"uc.login\",
		res.descripcion as \"res.descripcion\",
