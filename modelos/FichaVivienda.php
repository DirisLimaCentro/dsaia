<?php
//Incluímos inicialmente la conexión a la base de datos
require "../config/Conexion.php";

Class Ficha
{
	//Implementamos nue
	public function __construct()
	{

	}

	public function getCoordenadasFumigacion($desde,$hasta,$id_establecimiento)
	{
		$cri=" where fecha_programada between '$desde' and '$hasta' ";
		if ($id_establecimiento!=''){
			$cri.=" and f.id_local='$id_establecimiento' ";
		}

		/*$sql="select d.latitud,d.longitud,
			f.id_local,
			to_char(fecha_programada,'ddmmyyyy') as fecha_programada
			from ficha_rutas_fumigacion_detalle d
			inner join ficha_rutas_fumigacion f on f.id=d.id_ficha_fumigacion			
			where f.fecha_programada='$desde' order by f.id_local,f.fecha_programada,d.orden".$cri;*/

		$sql="select f.id_local,
			f.fecha_programada,
			(select json_agg(row_to_json(d.*)) from ficha_rutas_fumigacion_detalle d
			where d.id_ficha_fumigacion = f.id )::text coordenadas
			from ficha_rutas_fumigacion f ".$cri;	

		return ejecutarConsulta($sql);
	}

	public function getCoordenadasNebulizacion($desde,$hasta,$id_establecimiento)
	{
		$cri="";
		if ($id_establecimiento!=''){
			$cri.=" and id_local='$id_establecimiento' ";
		}

		$sql="select fd.latitud,fd.longitud,
		l.latitud as latitud_est,
		l.longitud as longitud_est,
		l.nombre as establecimiento,
		f.id_local
		from ficha_nebulizacion f
		inner join locales l on l.id=f.id_local
		inner join ficha_nebulizacion_detalle fd on f.id=fd.id_nebulizacion
		where f.fecha>='$desde' and f.fecha<='$hasta' and fd.id_condicion_vivienda='1' ".$cri." order by fd.fecha_creacion";

		//echo $sql;

		return ejecutarConsulta($sql);
	}




	public function getCoordenadas($desde,$hasta,$tipo,$id_establecimiento)
	{

		/*$sql="select fvd.latitud,fvd.longitud from 
		ficha_vivienda_detalle fvd
		inner join ficha_vivienda fv on fv.id=fvd.id_ficha_vivienda
		where 
		fv.fecha_inicio>='$desde' and fv.fecha_termino<='$hasta' and
		( (cnt_tipo_1_p+cnt_tipo_2_p+cnt_tipo_3_p+cnt_tipo_4_p+cnt_tipo_5_p+cnt_tipo_6_p+cnt_tipo_7_p+cnt_tipo_8_p+cnt_tipo_9_p+cnt_tipo_10_p)>0 
		or fvd.id_condicion_vivienda='6')
		and
		(latitud is not null and latitud<>'' and longitud<>'')";*/

		/*$sql="select fvd.latitud,fvd.longitud,
		fvd.id_condicion_vivienda,
		case when fvd.id_condicion_vivienda='6' then 'red' else 'green' end as color
		from 
		ficha_vivienda_detalle fvd
		inner join ficha_vivienda fv on fv.id=fvd.id_ficha_vivienda
		where 
		fv.fecha_inicio>='$desde' and fv.fecha_termino<='$hasta' and 
		fvd.id_condicion_vivienda in ('6','2') and 
		(latitud is not null and latitud<>'' and longitud<>'')";*/
		$cri="";
		if ($id_establecimiento!=''){
			$cri.=" and fv.id_local in ('".$id_establecimiento."') ";
		}

		if ($tipo=='X'){
			
			/*$sql="select fvd.latitud,fvd.longitud,
			fvd.id_condicion_vivienda,
			case when fvd.id_condicion_vivienda='6' then 'red' else 'green' end as color
			from 
			ficha_vivienda_detalle fvd
			inner join ficha_vivienda fv on fv.id=fvd.id_ficha_vivienda
			where 
			fv.fecha_inicio>='$desde' and fv.fecha_termino<='$hasta'  
			and fv.id_tipo_actividad in ('5','4')  and 
			(latitud is not null and latitud<>'' and longitud<>'') 			
			".$cri." order by fvd.id";*/
			$sql="select * from (
				select fv.id,fv.id_local,fv.fecha_inicio,
				(
				select json_agg (y) from (
				select row_to_json(x) 
				FROM (
				    SELECT latitud,longitud
				    from ficha_vivienda_detalle d where (COALESCE(latitud, '')<>'' and length(latitud)>5 ) and d.id_ficha_vivienda=fv.id
				) x
				)y
				) as coordenadas
				from ficha_vivienda fv  
				where fv.fecha_inicio>='$desde' and fv.fecha_termino<='$hasta'
				and fv.id_tipo_actividad in ('5') ".$cri."
				)z where coordenadas is not null";




		}else{
			$sql="select fvd.latitud,fvd.longitud,
			fvd.id_condicion_vivienda,
			case when fvd.id_condicion_vivienda='6' then 'red' else 'green' end as color
			from 
			ficha_vivienda_detalle fvd
			inner join ficha_vivienda fv on fv.id=fvd.id_ficha_vivienda
			where 
			fv.fecha_inicio>='$desde' and fv.fecha_termino<='$hasta' and 
			
			(
			cnt_tipo_1_p+
			cnt_tipo_2_p+
			cnt_tipo_3_p+
			cnt_tipo_4_p+
			cnt_tipo_5_p+
			cnt_tipo_6_p+
			cnt_tipo_7_p+
			cnt_tipo_8_p+
			cnt_tipo_9_p+
			cnt_tipo_10_p
			)>0			

			 and 
			(latitud is not null and latitud<>'' and longitud<>'') 			
			".$cri." order by fvd.id";	
		}

		//fvd.id_condicion_vivienda in ('6')

		return ejecutarConsulta($sql);
	}

	public function dataFumigacion($desde,$hasta,$tipo,$id_establecimiento){

		$cri="";
		if ($id_establecimiento!=''){
			$cri.=" and id_local='$id_establecimiento' ";
		}

		$sql="select fr.*,
		l.nombre as establecimiento
		from 
		ficha_rutas_fumigacion fr
		inner join locales l on l.id=fr.id_local
		where fecha_programada between '$desde' and '$hasta' ".$cri;

		return ejecutarConsulta($sql);
	}

	public function dataNebulizacion($desde,$hasta,$tipo,$id_establecimiento){

		$cri="";
		if ($id_establecimiento!=''){
			$cri.=" and f.id_local='$id_establecimiento' ";
		}

		$sql="
		select l.nombre as establecimiento,
		l.latitud,l.longitud,
		to_char(f.fecha,'dd/mm/yyyy') as fecha,
		count(*) as cantidad
		from ficha_nebulizacion f
		inner join ficha_nebulizacion_detalle fd on f.id=fd.id_nebulizacion
		inner join locales l on l.id=f.id_local		
		where f.fecha>='$desde' and f.fecha<='$hasta' and fd.id_condicion_vivienda='1' ".$cri." 
		group by 
		l.nombre,
		l.latitud,l.longitud,
		to_char(f.fecha,'dd/mm/yyyy')
		order by l.nombre";

		return ejecutarConsulta($sql);
	}

	public function positivosDistrito($desde,$hasta,$tipo,$id_establecimiento,$tipo_ag)
	{
		/*$sql="select 
		u.distrito,
		count(fvd.*) as cantidad
		from ficha_vivienda fv 
		inner join ficha_vivienda_detalle fvd on fv.id=fvd.id_ficha_vivienda
		inner JOIN locales l on l.id=fv.id_local
		inner join ubigeo u on u.id=l.id_ubigeo
		where fv.fecha_inicio>='$desde' and fv.fecha_termino<='$hasta'
		and 
		( (cnt_tipo_1_p+cnt_tipo_2_p+cnt_tipo_3_p+cnt_tipo_4_p+cnt_tipo_5_p+cnt_tipo_6_p+cnt_tipo_7_p+cnt_tipo_8_p+cnt_tipo_9_p+cnt_tipo_10_p)>0 
		or fvd.id_condicion_vivienda='6')
		and (latitud is not null and latitud<>'' and longitud<>'')
		group by u.distrito
		";*/
		$cri="";
		if ($tipo=='C'){
			//$cri=" and f.id_tipo_actividad in ('5','4') ";
		}

		if ($id_establecimiento!=''){
			$cri.=" and f.id_local in ('".$id_establecimiento."') ";
		}

		if ($tipo_ag=='D'){
			$sql="select distrito,confirmados,probables,
			fallecidos,
			febriles,

			autoctonos_confirmados,
			importados_confirmados,
			importados_probables,
			autoctonos_probables,
			indice_mapa,sum(inspeccionados) as inspeccionados,
			sum(positivos) as positivos,
			round((sum(positivos)/sum(inspeccionados))*100,3) as indice
			from (
			select u.distrito as  distrito,confirmados,probables,
			autoctonos_confirmados,
			importados_confirmados,
			importados_probables,
			autoctonos_probables,
			fallecidos,
			febriles,
			u.indice_mapa,count(d.*) as  inspeccionados,0 as positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			inner join ubigeo u on u.id=d.id_ubigeo
			where f.estado=true and f.fecha_inicio>='$desde' and f.fecha_termino<='$hasta' ".$cri."
			and (id_condicion_vivienda in ('2','6','7'))
			group by u.distrito,confirmados,probables,
			autoctonos_confirmados,
			importados_confirmados,
			importados_probables,
			autoctonos_probables,
			fallecidos,
			febriles,
			u.indice_mapa
			union all
			select u.distrito as  distrito,confirmados,probables,
			autoctonos_confirmados,
			importados_confirmados,
			importados_probables,
			autoctonos_probables,
			fallecidos,
			febriles,
			u.indice_mapa,0 as inspeccionados, count(d.*) as  positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			inner join ubigeo u on u.id=d.id_ubigeo
			where f.estado=true and f.fecha_inicio>='$desde' and f.fecha_termino<='$hasta' ".$cri."
			and 

			(
			cnt_tipo_1_p+
			cnt_tipo_2_p+
			cnt_tipo_3_p+
			cnt_tipo_4_p+
			cnt_tipo_5_p+
			cnt_tipo_6_p+
			cnt_tipo_7_p+
			cnt_tipo_8_p+
			cnt_tipo_9_p+
			cnt_tipo_10_p
			)>0

			group by u.distrito,confirmados,probables,
			autoctonos_confirmados,
			importados_confirmados,
			importados_probables,
			autoctonos_probables,
			fallecidos,
			febriles,			
			u.indice_mapa
			)x group by distrito,confirmados,
			autoctonos_confirmados,
			importados_confirmados,
			importados_probables,
			autoctonos_probables,	
			fallecidos,
			febriles,		
			probables,indice_mapa order by distrito";
		}else{
			$sql="select distrito,
			sum(confirmados) as confirmados,
			sum(probables) as probables,
			sum(fallecidos) as fallecidos,
			sum(febriles) as febriles,
			sum(autoctonos_confirmados) as autoctonos_confirmados,
			sum(importados_confirmados) as importados_confirmados,
			sum(importados_probables) as importados_probables,
			sum(autoctonos_probables) as autoctonos_probables,
			0 as indice_mapa,sum(inspeccionados) as inspeccionados,
			sum(positivos) as positivos,
			round((sum(positivos)/sum(inspeccionados))*100,3) as indice
			from (
			select l.ris as  distrito,confirmados,probables,
			autoctonos_confirmados,
			importados_confirmados,
			importados_probables,
			autoctonos_probables,
			fallecidos,
			febriles,
			u.indice_mapa,count(d.*) as  inspeccionados,0 as positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			inner join ubigeo u on u.id=l.id_ubigeo
			where f.estado=true and f.fecha_inicio>='$desde' and f.fecha_termino<='$hasta' ".$cri."
			and (id_condicion_vivienda in ('2','6','7'))
			group by l.ris,confirmados,probables,
			autoctonos_confirmados,
			importados_confirmados,
			importados_probables,
			autoctonos_probables,
			fallecidos,
			febriles,
			u.indice_mapa
			union all
			select l.ris as  distrito,confirmados,probables,
			autoctonos_confirmados,
			importados_confirmados,
			importados_probables,
			autoctonos_probables,
			fallecidos,
			febriles,
			u.indice_mapa,0 as inspeccionados, count(d.*) as  positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			inner join ubigeo u on u.id=l.id_ubigeo
			where f.estado=true and f.fecha_inicio>='$desde' and f.fecha_termino<='$hasta' ".$cri."
			and 
			(
			cnt_tipo_1_p+
			cnt_tipo_2_p+
			cnt_tipo_3_p+
			cnt_tipo_4_p+
			cnt_tipo_5_p+
			cnt_tipo_6_p+
			cnt_tipo_7_p+
			cnt_tipo_8_p+
			cnt_tipo_9_p+
			cnt_tipo_10_p
			)>0			
			group by l.ris,confirmados,probables,
			autoctonos_confirmados,
			importados_confirmados,
			importados_probables,
			autoctonos_probables,
			fallecidos,
			febriles,			
			u.indice_mapa
			)x group by distrito order by distrito";
		}
		
		//and (latitud is not null and latitud<>'' and longitud<>'')

		return ejecutarConsulta($sql);
	}

	public function reportCobertura($mes,$anio,$tipo){
		$filter="";
		$filter.=" and f.id_tipo_actividad in (";
		$aTipo=explode(",", $tipo);
		for ($i=0;$i<count($aTipo);$i++){
			$filter.="'".$aTipo[$i]."',";
		}
		$filter=substr($filter,0,strlen($filter)-1).") ";


		$sql="select establecimiento,sum(inspeccionados) as inspeccionados,sum(positivos) as positivos,sum(programados) as programados from (
			select l.nombre as  establecimiento,count(d.*) as  inspeccionados,0 as positivos,0 as programados
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			where f.estado=true and date_part('month',fecha_inicio)='$mes'
			and date_part('year',fecha_inicio)='$anio'
			and (id_condicion_vivienda in ('2','6','7')) ".$filter."
			group by l.nombre
			union all
			select l.nombre as  establecimiento,0 as inspeccionados, count(d.*) as  positivos,0 as programados
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			where f.estado=true and date_part('month',fecha_inicio)='$mes'
			and date_part('year',fecha_inicio)='$anio'
						
			and (
			cnt_tipo_1_p+
			cnt_tipo_2_p+
			cnt_tipo_3_p+
			cnt_tipo_4_p+
			cnt_tipo_5_p+
			cnt_tipo_6_p+
			cnt_tipo_7_p+
			cnt_tipo_8_p+
			cnt_tipo_9_p+
			cnt_tipo_10_p
			)>0

			".$filter."
			group by l.nombre
            union all
            select l.nombre as  establecimiento,0 as inspeccionados, 0 as  positivos,cb.cobertura as programados
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
            left join locales_coberturas cb on cb.id_local=f.id_local 
            and cb.nmes=date_part('month',fecha_inicio)::varchar 
            and cb.anio=date_part('year',fecha_inicio)::varchar and cb.tipo='$tipo' 
			where f.estado=true and date_part('month',fecha_inicio)='$mes'
			and date_part('year',fecha_inicio)='$anio' ".$filter."
			group by l.nombre,cb.cobertura    
			)x group by establecimiento order by establecimiento";
		return ejecutarConsulta($sql);
	}

	public function reportCoberturaSector($id_local,$mes,$anio,$tipo){

		$filter="";
		$filter.=" and f.id_tipo_actividad in (";
		$aTipo=explode(",", $tipo);
		for ($i=0;$i<count($aTipo);$i++){
			$filter.="'".$aTipo[$i]."',";
		}
		$filter=substr($filter,0,strlen($filter)-1).") ";

		$sql="select 'Sector '||sector as establecimiento,sum(inspeccionados) as inspeccionados,sum(positivos) as positivos,sum(programados) as programados from 
		(
		select sector,0 as inspeccionados,0 as positivos,round(viviendas /
		round(total/ round(((1.96*1.96)*(total*0.5*0.5))/((0.05*0.05)*(total-1)+(1.96*1.96)*0.5*0.5),0) ,2) 
		)as programados from (
		select l.nombre as establecimiento,l.id,s.descripcion as sector,sum(lo.cnt_viviendas) as viviendas,
		(select sum(cnt_viviendas)  from localidades where localidades.id_local=l.id ) as total
		from localidades lo
		inner join locales l on l.id=lo.id_local
		inner join tablas s on s.id_tipo=lo.sector::varchar and s.id_tabla='17'
		where l.id='$id_local' group by l.id,l.nombre,s.descripcion )x
		union all
		select s.descripcion as sector,count(d.*) as  inspeccionados,0 as positivos,0 as programados
		from 
		ficha_vivienda_detalle d
		inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
		inner join tablas s on s.id_tipo=f.sector::varchar and s.id_tabla='17'
		where f.estado=true and date_part('month',fecha_inicio)='$mes'
		and f.id_local='$id_local'
		and date_part('year',fecha_inicio)='$anio'
		and (id_condicion_vivienda in ('2','6','7'))
		".$filter."
		group by s.descripcion
		union all
		select s.descripcion as sector,0 as  inspeccionados,count(d.*) as positivos,0 as programados
		from 
		ficha_vivienda_detalle d
		inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
		inner join tablas s on s.id_tipo=f.sector::varchar and s.id_tabla='17'
		where f.estado=true and date_part('month',fecha_inicio)='$mes'
		and f.id_local='$id_local' 
		and date_part('year',fecha_inicio)='$anio'

		and (
			cnt_tipo_1_p+
			cnt_tipo_2_p+
			cnt_tipo_3_p+
			cnt_tipo_4_p+
			cnt_tipo_5_p+
			cnt_tipo_6_p+
			cnt_tipo_7_p+
			cnt_tipo_8_p+
			cnt_tipo_9_p+
			cnt_tipo_10_p
		)>0

		".$filter."
		group by s.descripcion
		)z
		group by sector order by 1";
		return ejecutarConsulta($sql);
	}




	public function reportAedicoSector($mes,$anio,$tipo,$nivel,$id_local){
		if ($nivel=='S') {
			$sql="select ris,sum(inspeccionados) as inspeccionados,sum(positivos) as positivos from (
			select s.descripcion as ris,count(d.*) as  inspeccionados,0 as positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			inner join tablas s on s.id_tipo=f.sector::varchar and s.id_tabla='17'

			where f.estado=true and date_part('month',fecha_inicio)='$mes'
			and date_part('year',fecha_inicio)='$anio'
			and f.id_local='$id_local'
			and (id_condicion_vivienda in ('2','6','7'))
			group by s.descripcion
			union all
			select s.descripcion as ris,0 as inspeccionados, count(d.*) as  positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			inner join tablas s on s.id_tipo=f.sector::varchar and s.id_tabla='17'
			where f.estado=true and date_part('month',fecha_inicio)='$mes'
			and date_part('year',fecha_inicio)='$anio'
			and f.id_local='$id_local'
			
			and (
			cnt_tipo_1_p+
			cnt_tipo_2_p+
			cnt_tipo_3_p+
			cnt_tipo_4_p+
			cnt_tipo_5_p+
			cnt_tipo_6_p+
			cnt_tipo_7_p+
			cnt_tipo_8_p+
			cnt_tipo_9_p+
			cnt_tipo_10_p
			)>0

			group by s.descripcion
		)x group by ris order by ris";
	}else{
		$sql="select ris,sum(inspeccionados) as inspeccionados,sum(positivos) as positivos from (
			select lo.descripcion as ris,count(d.*) as  inspeccionados,0 as positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			inner join localidades lo on lo.id=f.id_localidad
			where f.estado=true and date_part('month',fecha_inicio)='$mes'
			and date_part('year',fecha_inicio)='$anio'
			and f.id_local='$id_local'
			and (id_condicion_vivienda in ('2','6','7'))
			group by lo.descripcion
			union all
			select lo.descripcion as ris,0 as inspeccionados, count(d.*) as  positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			inner join localidades lo on lo.id=f.id_localidad
			where f.estado=true and date_part('month',fecha_inicio)='$mes'
			and date_part('year',fecha_inicio)='$anio'
			and f.id_local='$id_local'
			and (
			cnt_tipo_1_p+
			cnt_tipo_2_p+
			cnt_tipo_3_p+
			cnt_tipo_4_p+
			cnt_tipo_5_p+
			cnt_tipo_6_p+
			cnt_tipo_7_p+
			cnt_tipo_8_p+
			cnt_tipo_9_p+
			cnt_tipo_10_p
			)>0
			group by lo.descripcion
		)x group by ris order by ris";
	}
	return ejecutarConsulta($sql);
}

	public function reportAedico($mes,$anio,$tipo,$nivel,$id_local){

		$cri_mes=" and date_part('month',fecha_inicio) IN (";
		$aMes=explode(",", $mes);
		for ($i=0;$i<count($aMes);$i++){
			$cri_mes.="'".$aMes[$i]."',";
		}
		$cri_mes=substr($cri_mes,0,strlen($cri_mes)-1).") ";

		$cri_mes.=" and f.id_tipo_actividad in (";
		$aTipo=explode(",", $tipo);
		for ($i=0;$i<count($aTipo);$i++){
			$cri_mes.="'".$aTipo[$i]."',";
		}
		$cri_mes=substr($cri_mes,0,strlen($cri_mes)-1).") ";

		//and id_condicion_vivienda='6'


		if ($nivel=='R') {
			$sql="select ris,sum(inspeccionados) as inspeccionados,sum(positivos) as positivos from (
			select l.ris,count(d.*) as  inspeccionados,0 as positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			where f.estado=true ".$cri_mes."
			and date_part('year',fecha_inicio)='$anio'
			and (id_condicion_vivienda in ('2','6','7'))
			group by l.ris
			union all
			select l.ris,0 as inspeccionados, count(d.*) as  positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			where f.estado=true ".$cri_mes."
			and date_part('year',fecha_inicio)='$anio'
			and (
			cnt_tipo_1_p+
			cnt_tipo_2_p+
			cnt_tipo_3_p+
			cnt_tipo_4_p+
			cnt_tipo_5_p+
			cnt_tipo_6_p+
			cnt_tipo_7_p+
			cnt_tipo_8_p+
			cnt_tipo_9_p+
			cnt_tipo_10_p
			)>0
			group by l.ris
		)x group by ris order by ris";
	}elseif($nivel=='E'){
		
			$sql="select ris,sum(inspeccionados) as inspeccionados,sum(positivos) as positivos from (
			select ris,sum(inspeccionados) as inspeccionados,sum(positivos) as positivos from (
			select l.nombre as  ris,count(d.*) as  inspeccionados,0 as positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			where f.estado=true ".$cri_mes."
			and date_part('year',fecha_inicio)='$anio'
			and (id_condicion_vivienda in ('2','6','7'))
			group by l.nombre
			union all
			select l.nombre as  ris,0 as inspeccionados, count(d.*) as  positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			where f.estado=true ".$cri_mes."
			and date_part('year',fecha_inicio)='$anio'
			and (
			cnt_tipo_1_p+
			cnt_tipo_2_p+
			cnt_tipo_3_p+
			cnt_tipo_4_p+
			cnt_tipo_5_p+
			cnt_tipo_6_p+
			cnt_tipo_7_p+
			cnt_tipo_8_p+
			cnt_tipo_9_p+
			cnt_tipo_10_p
			)>0
			group by l.nombre
			)x group by ris 
			union all
			select nombre as ris,0 as inspeccionados,0 as positivos
			from locales where  ris is not null 
		)z group by ris order by ris";
	}elseif($nivel=='D'){
		$sql="select ris,sum(inspeccionados) as inspeccionados,sum(positivos) as positivos from (
			select u.distrito as  ris,count(d.*) as  inspeccionados,0 as positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			inner join ubigeo u on u.id=d.id_ubigeo
			where f.estado=true ".$cri_mes."
			and date_part('year',fecha_inicio)='$anio'
			and (id_condicion_vivienda in ('2','6','7'))
			group by u.distrito
			union all
			select u.distrito as  ris,0 as inspeccionados, count(d.*) as  positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			inner join ubigeo u on u.id=d.id_ubigeo
			where f.estado=true ".$cri_mes."
			and date_part('year',fecha_inicio)='$anio'
			and (
			cnt_tipo_1_p+
			cnt_tipo_2_p+
			cnt_tipo_3_p+
			cnt_tipo_4_p+
			cnt_tipo_5_p+
			cnt_tipo_6_p+
			cnt_tipo_7_p+
			cnt_tipo_8_p+
			cnt_tipo_9_p+
			cnt_tipo_10_p
			)>0
			group by u.distrito
			)x group by ris order by ris";
	}elseif($nivel=='S'){
			$sql="select ris,sum(inspeccionados) as inspeccionados,sum(positivos) as positivos from (
			select s.descripcion as ris,count(d.*) as  inspeccionados,0 as positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			inner join tablas s on s.id_tipo=f.sector::varchar and s.id_tabla='17'
			where f.estado=true ".$cri_mes."
			and date_part('year',fecha_inicio)='$anio'
			and f.id_local='$id_local'
			and (id_condicion_vivienda in ('2','6','7'))
			group by s.descripcion
			union all
			select s.descripcion as ris,0 as inspeccionados, count(d.*) as  positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			inner join tablas s on s.id_tipo=f.sector::varchar and s.id_tabla='17'
			where f.estado=true ".$cri_mes."
			and date_part('year',fecha_inicio)='$anio'
			and f.id_local='$id_local'
			and (
			cnt_tipo_1_p+
			cnt_tipo_2_p+
			cnt_tipo_3_p+
			cnt_tipo_4_p+
			cnt_tipo_5_p+
			cnt_tipo_6_p+
			cnt_tipo_7_p+
			cnt_tipo_8_p+
			cnt_tipo_9_p+
			cnt_tipo_10_p
			)>0
			group by s.descripcion
		)x group by ris order by ris";
	}else{
		$sql="select ris,sum(inspeccionados) as inspeccionados,sum(positivos) as positivos from (
			select lo.descripcion as ris,count(d.*) as  inspeccionados,0 as positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			inner join localidades lo on lo.id=f.id_localidad
			where f.estado=true ".$cri_mes."
			and date_part('year',fecha_inicio)='$anio'
			and f.id_local='$id_local'
			and (id_condicion_vivienda in ('2','6','7'))
			group by lo.descripcion
			union all
			select lo.descripcion as ris,0 as inspeccionados, count(d.*) as  positivos
			from 
			ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			inner join localidades lo on lo.id=f.id_localidad
			where f.estado=true ".$cri_mes."
			and date_part('year',fecha_inicio)='$anio'
			and f.id_local='$id_local'
			and (
			cnt_tipo_1_p+
			cnt_tipo_2_p+
			cnt_tipo_3_p+
			cnt_tipo_4_p+
			cnt_tipo_5_p+
			cnt_tipo_6_p+
			cnt_tipo_7_p+
			cnt_tipo_8_p+
			cnt_tipo_9_p+
			cnt_tipo_10_p
			)>0
			group by lo.descripcion
			)x group by ris order by ris";
	}
	return ejecutarConsulta($sql);
}
	public function reportConsolidadoAedes($mes,$anio,$tipo,$id_nivel,$id_local){
		$let=strtolower($tipo);

		if ($id_nivel=='R'){  //RIS
			$sql="select l.ris,
			sum(cnt_tipo_1_".$let.") as tot1,
			sum(cnt_tipo_2_".$let.") as tot2,
			sum(cnt_tipo_3_".$let.") as tot3,
			sum(cnt_tipo_4_".$let.") as tot4,
			sum(cnt_tipo_5_".$let.") as tot5,
			sum(cnt_tipo_6_".$let.") as tot6,
			sum(cnt_tipo_7_".$let.") as tot7,
			sum(cnt_tipo_8_".$let.") as tot8,
			sum(cnt_tipo_9_".$let.") as tot9
			from ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			where f.estado=true and date_part('month',fecha_inicio)='$mes'
			and date_part('year',fecha_inicio)='$anio'
			group by l.ris order by l.ris";
		}elseif($id_nivel=='E'){ //Establecimiento
			$sql="select l.nombre as ris,
			sum(cnt_tipo_1_".$let.") as tot1,
			sum(cnt_tipo_2_".$let.") as tot2,
			sum(cnt_tipo_3_".$let.") as tot3,
			sum(cnt_tipo_4_".$let.") as tot4,
			sum(cnt_tipo_5_".$let.") as tot5,
			sum(cnt_tipo_6_".$let.") as tot6,
			sum(cnt_tipo_7_".$let.") as tot7,
			sum(cnt_tipo_8_".$let.") as tot8,
			sum(cnt_tipo_9_".$let.") as tot9
			from ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			where f.estado=true and date_part('month',fecha_inicio)='$mes'
			and date_part('year',fecha_inicio)='$anio'
			group by l.nombre order by 1";

		}elseif($id_nivel=='D'){    //Distrito
			$sql="select u.distrito as ris,
			sum(cnt_tipo_1_".$let.") as tot1,
			sum(cnt_tipo_2_".$let.") as tot2,
			sum(cnt_tipo_3_".$let.") as tot3,
			sum(cnt_tipo_4_".$let.") as tot4,
			sum(cnt_tipo_5_".$let.") as tot5,
			sum(cnt_tipo_6_".$let.") as tot6,
			sum(cnt_tipo_7_".$let.") as tot7,
			sum(cnt_tipo_8_".$let.") as tot8,
			sum(cnt_tipo_9_".$let.") as tot9
			from ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			inner join ubigeo u on u.id=l.id_ubigeo
			where f.estado=true and date_part('month',fecha_inicio)='$mes'
			and date_part('year',fecha_inicio)='$anio'
			group by u.distrito order by 1";
		}elseif($id_nivel=='S'){
			$sql="select s.descripcion as ris,
			sum(cnt_tipo_1_".$let.") as tot1,
			sum(cnt_tipo_2_".$let.") as tot2,
			sum(cnt_tipo_3_".$let.") as tot3,
			sum(cnt_tipo_4_".$let.") as tot4,
			sum(cnt_tipo_5_".$let.") as tot5,
			sum(cnt_tipo_6_".$let.") as tot6,
			sum(cnt_tipo_7_".$let.") as tot7,
			sum(cnt_tipo_8_".$let.") as tot8,
			sum(cnt_tipo_9_".$let.") as tot9
			from ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			inner join tablas s on s.id_tipo=f.sector::varchar and s.id_tabla='17'
			where f.estado=true and date_part('month',fecha_inicio)='$mes' and f.id_local='$id_local' 
			and date_part('year',fecha_inicio)='$anio'
			group by s.descripcion order by 1";
		}elseif($id_nivel=='L'){
			$sql="select lo.descripcion as ris,
			sum(cnt_tipo_1_".$let.") as tot1,
			sum(cnt_tipo_2_".$let.") as tot2,
			sum(cnt_tipo_3_".$let.") as tot3,
			sum(cnt_tipo_4_".$let.") as tot4,
			sum(cnt_tipo_5_".$let.") as tot5,
			sum(cnt_tipo_6_".$let.") as tot6,
			sum(cnt_tipo_7_".$let.") as tot7,
			sum(cnt_tipo_8_".$let.") as tot8,
			sum(cnt_tipo_9_".$let.") as tot9
			from ficha_vivienda_detalle d
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			inner join localidades lo on lo.id=f.id_localidad
			where f.estado=true and date_part('month',fecha_inicio)='$mes' and f.id_local='$id_local' 
			and date_part('year',fecha_inicio)='$anio'
			group by lo.descripcion order by 1";
		}



		return ejecutarConsulta($sql);

	}

	public function reportCalidadAgua($anio){
		
		$sql="select split_part(keyo,'|',1) as ris, split_part(keyo,'|',2) as establecimiento, 
sum(\"1\") as \"1\",
sum(\"2\") as \"2\",
sum(\"3\") as \"3\",
sum(\"4\") as \"4\",
sum(\"5\") as \"5\",
sum(\"6\") as \"6\",
sum(\"7\") as \"7\",
sum(\"8\") as \"8\",
sum(\"9\") as \"9\",
sum(\"10\") as \"10\",
sum(\"11\") as \"11\",
sum(\"12\") as \"12\"
from (
    
select * 
from crosstab (
    'select concat(l.ris,''|'',l.nombre) as keyo,f.nro_mes,count(f.*) as cantidad from ficha_cloro
    f inner join locales l on l.id=f.id_local
    where f.estado is true and f.nro_anio=".$anio." and l.ris is not null 
    group by l.ris,l.nombre,f.nro_mes',
    'select generate_series(1,12)::varchar as nro_mes'
)as(
    keyo text,
    \"1\" int,
    \"2\" int,
    \"3\" int,
    \"4\" int,
    \"5\" int,
    \"6\" int,
    \"7\" int,
    \"8\" int,
    \"9\" int,
    \"10\" int,
    \"11\" int,
    \"12\" int
)
union all
select concat(ris,'|',nombre) as keyo,
0 as \"1\",
0 as \"2\",
0 as \"3\",
0 as \"4\",
0 as \"5\",
0 as \"6\",
0 as \"7\",
0 as \"8\",
0 as \"9\",
0 as \"10\",
0 as \"11\",
0 as \"12\" from locales where ris is not null
    
)x 
group by split_part(keyo,'|',1), split_part(keyo,'|',2)
order by 1,2    ";

		return ejecutarConsulta($sql);

	}

	public function resumenAedesDetalle($mes,$anio,$id_establecimiento,$tipo,$desde,$hasta){

		$cri_cat="";
		if ($id_establecimiento!='*'){
			$cri_cat=" AND f.id_local IN (";
			$aProv=explode(",", $id_establecimiento);
			for ($i=0;$i<count($aProv);$i++){
				$cri_cat.="".$aProv[$i].",";
			}
			$cri_cat=substr($cri_cat,0,strlen($cri_cat)-1).") ";
		}


		if ($tipo=='M'){
			$cri_mes=" and date_part('year',fecha_inicio)='".$anio."' ";
			if ($mes!='*'){
				$cri_mes.=" and date_part('month',fecha_inicio) IN (";
				$aMes=explode(",", $mes);
				for ($i=0;$i<count($aMes);$i++){
					$cri_mes.="'".$aMes[$i]."',";
				}
				$cri_mes=substr($cri_mes,0,strlen($cri_mes)-1).") ";
			}	
		}else{
			$cri_mes=" and f.fecha_inicio>='".$_GET['desde']."' and f.fecha_termino<='".$_GET['hasta']."' ";
		}
		



		$sql="select 
		date_part('year',f.fecha_inicio) as anio,
		to_char(f.fecha_inicio,'TMMonth') as mes,
		a.descripcion as actividad,
		tdoc.descripcion as tipo_documento,
		f.numero_documento_paciente,
		f.nombres_paciente,
		tcon.descripcion as tipo_contagio,
		u.distrito,
		l.nombre as establecimiento,
		l.ris,
		f.sector,
		loc.descripcion as localidad,
		to_char(f.fecha_inicio,'dd/mm/yyyy') as fecha_inicio,
		to_char(f.fecha_termino,'dd/mm/yyyy') as fecha_termino,
		fd.direccion_familia,
		fd.latitud,
		fd.longitud,
		fd.nro_habitantes,		

		case when (
			cnt_tipo_1_p+
			cnt_tipo_2_p+
			cnt_tipo_3_p+
			cnt_tipo_4_p+
			cnt_tipo_5_p+
			cnt_tipo_6_p+
			cnt_tipo_7_p+
			cnt_tipo_8_p+
			cnt_tipo_9_p+
			cnt_tipo_10_p
			)>0	

		then 'VIVIENDA POSITIVA' else tc.descripcion end as condicion_vivienda,

		fd.cnt_tipo_1_i,
		fd.cnt_tipo_1_p,
		fd.cnt_tipo_1_t,
		fd.cnt_tipo_1_tf,

		fd.cnt_tipo_2_i,
		fd.cnt_tipo_2_p,
		fd.cnt_tipo_2_t,
		fd.cnt_tipo_2_tf,

		fd.cnt_tipo_3_i,
		fd.cnt_tipo_3_p,
		fd.cnt_tipo_3_t,
		fd.cnt_tipo_3_tf,

		fd.cnt_tipo_10_i,
		fd.cnt_tipo_10_p,
		fd.cnt_tipo_10_t,
		fd.cnt_tipo_10_tf,

		fd.cnt_tipo_4_i,
		fd.cnt_tipo_4_p,
		fd.cnt_tipo_4_t,
		fd.cnt_tipo_4_tf,

		fd.cnt_tipo_5_i,
		fd.cnt_tipo_5_p,
		fd.cnt_tipo_5_t,
		fd.cnt_tipo_5_tf,

		fd.cnt_tipo_6_i,
		fd.cnt_tipo_6_p,
		fd.cnt_tipo_6_t,
		fd.cnt_tipo_6_tf,

		fd.cnt_tipo_7_i,
		fd.cnt_tipo_7_p,
		fd.cnt_tipo_7_t,
		fd.cnt_tipo_7_tf,
		fd.cnt_tipo_7_d,

		fd.cnt_tipo_9_i,
		fd.cnt_tipo_9_p,
		fd.cnt_tipo_9_t,
		fd.cnt_tipo_9_tf,
		fd.cnt_tipo_9_d,		

		fd.cnt_larvicidas,
		fd.cnt_febriles,		
		(per.ape_paterno||' '||per.ape_paterno||' '||per.nombres) as usuario_inspector,
		fd.cantidad_probable_dengue				
		from ficha_vivienda f
		inner join ficha_vivienda_detalle fd on fd.id_ficha_vivienda=f.id
		inner join locales l on l.id=f.id_local
		inner join tablas a on a.id_tipo=f.id_tipo_actividad and a.id_tabla='18'
		inner join ubigeo u on u.id=fd.id_ubigeo
		inner join tablas tc on tc.id_tipo=fd.id_condicion_vivienda and tc.id_tabla='19'
		left join localidades loc on loc.id=f.id_localidad
		left join personal per on per.id=f.id_usuario_inspector
		left join tablas tcon on tcon.id_tipo=f.id_tipo_paciente and tcon.id_tabla='92'
		left join tablas tdoc on tdoc.id_tipo=f.id_tipo_documento_paciente and tdoc.id_tabla='1'
		where f.estado=true ".$cri_cat.$cri_mes."
		order by 1,	date_part('month',f.fecha_inicio),l.nombre,f.fecha_inicio";

		return ejecutarConsulta($sql);

	}


	public function resumenAedes($mes,$anio,$id_establecimiento,$tipo,$desde,$hasta){

		$cri_est="";

		if ($id_establecimiento!='*'){
			$cri_est=" AND f.id_local IN (";
			$aProv=explode(",", $id_establecimiento);
			for ($i=0;$i<count($aProv);$i++){
				$cri_est.="".$aProv[$i].",";
			}
			$cri_est=substr($cri_est,0,strlen($cri_est)-1).") ";
		}

		$cri_mes="";
		$cri_month="";
		$cri_date="";
		$cri_fecha="";

		if ($tipo=="M"){
			$cri_month.=" and date_part(''year'',fecha_inicio)=".$anio;
			$cri_mes.=" and date_part('year',fecha_inicio)=".$anio;

			if ($mes!='*'){
				$cri_month.=" and date_part(''month'',fecha_inicio) IN (";
				$cri_mes.=" and date_part('month',fecha_inicio) IN (";
				$aMes=explode(",", $mes);
				for ($i=0;$i<count($aMes);$i++){
					$cri_month.="".$aMes[$i].",";
					$cri_mes.="'".$aMes[$i]."',";
				}
				$cri_month=substr($cri_month,0,strlen($cri_month)-1).") ";
				$cri_mes=substr($cri_mes,0,strlen($cri_mes)-1).") ";
			}
		}else{
			$cri_date=" and fecha_inicio>=''".$desde."'' and fecha_termino<=''".$hasta."'' ";
			$cri_fecha=" and fecha_inicio>='".$desde."' and fecha_termino<='".$hasta."' ";
		}	

		//echo $cri_cat;
		/*
			select concat(date_part(''year'',f.fecha_inicio),''|'',
		substring(to_char(f.fecha_inicio,''TMMonth''),1,3),''|'',a.descripcion,''|'',
		u.distrito,''|'',l.nombre,''|'',l.ris) as keyo,		
		tc.descripcion as condicion_vivienda,
		count(fd.id_condicion_vivienda) as cnt_condicion_vivienda
		from ficha_vivienda f
		inner join ficha_vivienda_detalle fd on fd.id_ficha_vivienda=f.id
		inner join locales l on l.id=f.id_local
		inner join tablas a on a.id_tipo=f.id_tipo_actividad and a.id_tabla=''18''
		inner join ubigeo u on u.id=fd.id_ubigeo
		inner join tablas tc on tc.id_tipo=fd.id_condicion_vivienda and tc.id_tabla=''19''
		where f.estado=true ".$cri_est.$cri_month.$cri_date." 
		group by date_part(''year'',f.fecha_inicio),to_char(f.fecha_inicio,''TMMonth''),
		a.descripcion,u.distrito,l.nombre,l.ris,
		tc.descripcion
		*/


		$sql="select  split_part(keyo,'|',1) as anio, split_part(keyo,'|',2) as mes,
		split_part(keyo,'|',3) as actividad, split_part(keyo,'|',4) as distrito, split_part(keyo,'|',5) as establecimiento,
		split_part(keyo,'|',6) as ris,
		
		sum(\"VIVIENDAS INSPECCIONADAS\") as \"viviendas_inspeccionadas\",
		sum(\"CASAS RENUENTES\") as \"casas_renuentes\",
		sum(\"CASAS CERRADAS\") as \"casas_cerradas\",
		sum(\"CASAS DESHABITADAS\") as \"casas_deshabitadas\",
		sum(\"VIVIENDA POSITIVA\") as \"vivienda_positivas\",
		sum(\"VIVIENDA TRATADA CON LARVICIDA\") as \"vivienda_larvicidas\",
		sum(\"DEPOSITOS DESTRUIDOS\") as \"depositos_destruidos\",
		sum(nro_habitantes) as nro_habitantes,
		sum(tot_i_1) as tot_i_1,
		sum(tot_p_1) as tot_p_1,
		sum(tot_t_1) as tot_t_1,

		sum(tot_tf_1) as tot_tf_1,

		sum(tot_i_2) as tot_i_2,
		sum(tot_p_2) as tot_p_2,
		sum(tot_t_2) as tot_t_2,

		sum(tot_tf_2) as tot_tf_2,


		sum(tot_i_3) as tot_i_3,
		sum(tot_p_3) as tot_p_3,
		sum(tot_t_3) as tot_t_3,

		sum(tot_tf_3) as tot_tf_3,


		sum(tot_i_10) as tot_i_10,
		sum(tot_p_10) as tot_p_10,
		sum(tot_t_10) as tot_t_10,

		sum(tot_tf_10) as tot_tf_10,


		sum(tot_i_4) as tot_i_4,
		sum(tot_p_4) as tot_p_4,
		sum(tot_t_4) as tot_t_4,

		sum(tot_tf_4) as tot_tf_4,		

		sum(tot_i_5) as tot_i_5,
		sum(tot_p_5) as tot_p_5,
		sum(tot_t_5) as tot_t_5,

		sum(tot_tf_5) as tot_tf_5,


		sum(tot_i_6) as tot_i_6,
		sum(tot_p_6) as tot_p_6,
		sum(tot_t_6) as tot_t_6,

		sum(tot_tf_6) as tot_tf_6,


		sum(tot_i_7) as tot_i_7,
		sum(tot_p_7) as tot_p_7,
		sum(tot_t_7) as tot_t_7,

		sum(tot_tf_7) as tot_tf_7,


		sum(tot_d_7) as tot_d_7,



		sum(tot_i_9) as tot_i_9,
		sum(tot_p_9) as tot_p_9,
		sum(tot_t_9) as tot_t_9,

		sum(tot_tf_9) as tot_tf_9,
		sum(tot_d_9) as tot_d_9,	
		sum(tot_larv) as tot_larv



		from (
		select *,
		0 nro_habitantes,
		0 tot_i_1,
		0 tot_p_1,
		0 tot_t_1,

		0 tot_tf_1,

		0 tot_i_2,
		0 tot_p_2,
		0 tot_t_2,

		0 tot_tf_2,


		0 tot_i_3,
		0 tot_p_3,
		0 tot_t_3,

		0 tot_tf_3,


		0 tot_i_10,
		0 tot_p_10,
		0 tot_t_10,

		0 tot_tf_10,		

		0 tot_i_4,
		0 tot_p_4,
		0 tot_t_4,

		0 tot_tf_4,


		0 tot_i_5,
		0 tot_p_5,
		0 tot_t_5,

		0 tot_tf_5,		

		0 tot_i_6,
		0 tot_p_6,
		0 tot_t_6,

		0 tot_tf_6,		

		0 tot_i_7,
		0 tot_p_7,
		0 tot_t_7,
		0 tot_tf_7,		
		0 tot_d_7,

		0 tot_i_9,
		0 tot_p_9,
		0 tot_t_9,
		0 tot_tf_9,
		0 tot_d_9,
		0 tot_larv		

		from crosstab (
		'
		select keyo,condicion_vivienda,count(*) as cnt_condicion_vivienda from (
		select concat(date_part(''year'',f.fecha_inicio),''|'',
		substring(to_char(f.fecha_inicio,''TMMonth''),1,3),''|'',a.descripcion,''|'',
		u.distrito,''|'',l.nombre,''|'',l.ris) as keyo,

		''VIVIENDA POSITIVA''  as condicion_vivienda		
		
		from ficha_vivienda f
		inner join ficha_vivienda_detalle fd on fd.id_ficha_vivienda=f.id
		inner join locales l on l.id=f.id_local
		inner join tablas a on a.id_tipo=f.id_tipo_actividad and a.id_tabla=''18''
		inner join ubigeo u on u.id=fd.id_ubigeo
		inner join tablas tc on tc.id_tipo=fd.id_condicion_vivienda and tc.id_tabla=''19''
		where 

		(cnt_tipo_1_p+cnt_tipo_2_p+cnt_tipo_3_p+cnt_tipo_4_p+cnt_tipo_5_p+cnt_tipo_6_p+	cnt_tipo_7_p+cnt_tipo_8_p+cnt_tipo_9_p)>0

		and f.estado=true ".$cri_est.$cri_month.$cri_date." 

		union all
		
		select concat(date_part(''year'',f.fecha_inicio),''|'',
		substring(to_char(f.fecha_inicio,''TMMonth''),1,3),''|'',a.descripcion,''|'',
		u.distrito,''|'',l.nombre,''|'',l.ris) as keyo,
		case when (
		cnt_tipo_1_p+
		cnt_tipo_2_p+
		cnt_tipo_3_p+
		cnt_tipo_4_p+
		cnt_tipo_5_p+
		cnt_tipo_6_p+
		cnt_tipo_7_p+
		cnt_tipo_8_p+
		cnt_tipo_9_p
		)
		>0 then ''VIVIENDAS INSPECCIONADAS'' else tc.descripcion end as condicion_vivienda		
		from ficha_vivienda f
		inner join ficha_vivienda_detalle fd on fd.id_ficha_vivienda=f.id
		inner join locales l on l.id=f.id_local
		inner join tablas a on a.id_tipo=f.id_tipo_actividad and a.id_tabla=''18''
		inner join ubigeo u on u.id=fd.id_ubigeo
		inner join tablas tc on tc.id_tipo=fd.id_condicion_vivienda and tc.id_tabla=''19''
		where f.estado=true ".$cri_est.$cri_month.$cri_date." 

		)x		
		group by keyo,condicion_vivienda


		','select descripcion as condicion_vivienda from tablas where id_tabla=''19'' order by id_tipo::int') as (
		keyo text,
		
		\"VIVIENDAS INSPECCIONADAS\" int,
		\"CASAS RENUENTES\" int,
		\"CASAS CERRADAS\" int,
		\"CASAS DESHABITADAS\" int,
		\"VIVIENDA POSITIVA\" int,
		\"VIVIENDA TRATADA CON LARVICIDA\" int,
		\"DEPOSITOS DESTRUIDOS\" int) 
		union all

		select concat(date_part('year',f.fecha_inicio),'|',
		substring(to_char(f.fecha_inicio,'TMMonth'),1,3),'|',a.descripcion,'|',
		u.distrito,'|',l.nombre,'|',l.ris) as keyo,
		
		0 as \"VIVIENDAS INSPECCIONADAS\",
		0 as \"CASAS RENUENTES\",
		0 as \"CASAS CERRADAS\",
		0 as \"CASAS DESHABITADAS\",
		0 as \"VIVIENDA POSITIVA\",
		0 as \"VIVIENDA TRATADA CON LARVICIDA\",
		0 as \"DEPOSITOS DESTRUIDOS\",
		sum(nro_habitantes) as nro_habitantes,
		sum(cnt_tipo_1_i) as tot_i_1,
		sum(cnt_tipo_1_p) as tot_p_1,
		sum(cnt_tipo_1_t) as tot_t_1,

		sum(cnt_tipo_1_tf) as tot_tf_1,


		sum(cnt_tipo_2_i) as tot_i_2,
		sum(cnt_tipo_2_p) as tot_p_2,
		sum(cnt_tipo_2_t) as tot_t_2,

		sum(cnt_tipo_2_tf) as tot_tf_2,	

		sum(cnt_tipo_3_i) as tot_i_3,
		sum(cnt_tipo_3_p) as tot_p_3,
		sum(cnt_tipo_3_t) as tot_t_3,

		sum(cnt_tipo_3_tf) as tot_tf_3,		


		sum(cnt_tipo_10_i) as tot_i_10,
		sum(cnt_tipo_10_p) as tot_p_10,
		sum(cnt_tipo_10_t) as tot_t_10,

		sum(cnt_tipo_10_tf) as tot_tf_10,


		sum(cnt_tipo_4_i) as tot_i_4,
		sum(cnt_tipo_4_p) as tot_p_4,
		sum(cnt_tipo_4_t) as tot_t_4,

		sum(cnt_tipo_4_tf) as tot_tf_4,		

		sum(cnt_tipo_5_i) as tot_i_5,
		sum(cnt_tipo_5_p) as tot_p_5,
		sum(cnt_tipo_5_t) as tot_t_5,

		sum(cnt_tipo_5_tf) as tot_tf_5,



		sum(cnt_tipo_6_i) as tot_i_6,
		sum(cnt_tipo_6_p) as tot_p_6,
		sum(cnt_tipo_6_t) as tot_t_6,

		sum(cnt_tipo_6_tf) as tot_tf_6,


		sum(cnt_tipo_7_i) as tot_i_7,
		sum(cnt_tipo_7_p) as tot_p_7,
		sum(cnt_tipo_7_t) as tot_t_7,
		sum(cnt_tipo_7_tf) as tot_tf_7,		
		sum(cnt_tipo_7_d) as tot_d_7,


		sum(cnt_tipo_9_i) as tot_i_9,
		sum(cnt_tipo_9_p) as tot_p_9,
		sum(cnt_tipo_9_t) as tot_t_9,
		sum(cnt_tipo_9_tf) as tot_tf_9,
		sum(cnt_tipo_9_d) as tot_d_9,
		sum(cnt_larvicidas) as tot_larv

		from ficha_vivienda f
		inner join ficha_vivienda_detalle fd on fd.id_ficha_vivienda=f.id
		inner join locales l on l.id=f.id_local
		inner join tablas a on a.id_tipo=f.id_tipo_actividad and a.id_tabla='18'
		inner join ubigeo u on u.id=fd.id_ubigeo
		where f.estado=true ".$cri_est.$cri_mes.$cri_fecha." 
		group by date_part('year',f.fecha_inicio),to_char(f.fecha_inicio,'TMMonth'),
		a.descripcion,u.distrito,l.nombre,l.ris
	) x group by keyo order by 1,2,5 ";
	return ejecutarConsulta($sql);
}





	public function mostrarItem($id_ingreso,$id_item,$id_lote){
		$sql="SELECT
		d.id_item,cat.nombre as item,d.factor,d.cantidad,d.costo_unitario_umc,
		d.id_lote,l.numero_lote,to_char(l.fecha_vencimiento,'dd/mm/yyyy') as fecha_vencimiento,
		c.descripcion AS categoria,
		m.descripcion AS marca,
		ums.descripcion AS unidad_medida_salida,
		umi.descripcion AS unidad_medida_ingreso
		FROM
		ingresos_detalle d
		INNER JOIN items i ON d.id_item=i.id
		INNER JOIN catalogo cat ON cat.id=i.id_catalogo
		LEFT JOIN tablas c ON cat.id_grupo=c.id_tipo AND c.id_tabla='4'
		LEFT JOIN tablas m ON i.id_marca=m.id_tipo AND m.id_tabla='5'
		LEFT JOIN tablas ums ON i.id_medida_sal=ums.id_tipo AND ums.id_tabla='6'
		LEFT JOIN tablas umi ON i.id_medida_ing=umi.id_tipo AND umi.id_tabla='6'
		LEFT JOIN lotes l ON l.id=d.id_lote
		WHERE d.id_ingreso='$id_ingreso' AND i.id='$id_item' and d.id_lote='$id_lote' ";
		return ejecutarConsultaSimpleFila($sql);
	}
	public function verificaDisponibilidadLote($id_lote){
		$sql="SELECT  id_lote FROM salidas_detalle WHERE id_lote='$id_lote' LIMIT 1";
		return ejecutarConsulta($sql);
	}
	public function eliminaItem($id){
		$sql="delete from ficha_vivienda_detalle where id='$id'";
		return ejecutarConsulta($sql);
	}

	public function actualizarInsertarItem($accion,$id_ingreso,$id_item,$id_lote,$numero_lote,$cantidad,$costo_umc,$fecha_vencimiento,$factor){


		if ($fecha_vencimiento==''){
			$param_fv='';
		}else{
			$param_fv=",'".$fecha_vencimiento."'";
		}

		if ($accion=='U'){
			$sql="SELECT sp_update_item_compra('$id_ingreso','$id_item',".(empty($id_lote)?'0':$id_lote).",'$numero_lote','$cantidad','$costo_umc'".$param_fv.")";
		}else{
			$sql="SELECT sp_insert_item_compra('$id_ingreso','$id_item',".(empty($id_lote)?'null':$id_lote).",'$numero_lote','$cantidad','$costo_umc','$factor'".$param_fv.")";
		}
		return ejecutarConsulta($sql);
	}

	public function actualizar($id_ingreso,$id_local,$id_tipo_documento,$serie_documento,$numero_documento,
		$fecha_compra,$id_moneda,$id_forma_pago,$tipo_cambio,$porcentaje_igv,$numero_guia,$id_proveedor,$observacion,$id_usuario){

		$sql="SELECT sp_update_compra('$id_ingreso','$id_local','$id_tipo_documento','$serie_documento','$numero_documento','$fecha_compra','$id_moneda','$id_forma_pago','$tipo_cambio','$porcentaje_igv','$numero_guia','$id_proveedor','$observacion','$id_usuario')";

		return ejecutarConsulta($sql);

	}

	public function save($id_ficha,$id_local,$sector,$fecha_inicio,	$id_tipo_actividad,
		$id_localidad,$id_usuario,
		$fecha_termino,
		$id_tipo_documento_paciente,
		$numero_documento_paciente,
		$nombres_paciente,
		$id_tipo_paciente,
		$id_usuario_inspector,
		$direccion_ovitrampa

		){

		$sql="SELECT sp_crud_header_ficha_vivienda(
			'$id_ficha',
			'$id_local',
			'$sector',
			'$fecha_inicio',
			'$id_tipo_actividad',
			'$id_localidad',
			'$id_usuario',
			'$fecha_termino',
			'$id_tipo_documento_paciente',
			'$numero_documento_paciente',
			'$nombres_paciente',
			'$id_tipo_paciente',
			'$id_usuario_inspector',
			'$direccion_ovitrampa'

			)";

		return ejecutarConsulta($sql);

	}

	public function saveFichaDetalle($id_ficha,$id_ficha_detalle,$detalle,$id_ubigeo){

		$sql="SELECT sp_crud_body_ficha_vivienda_new('$id_ficha','$id_ficha_detalle','$detalle','$id_ubigeo')";

		return ejecutarConsulta($sql);

	}



	//Implementamos un método para insertar registros
	public function insertar_($nombre,$id_empresa,$id_marca,$id_categoria,$id_ums,$id_umi,$factor,$precio_compra,$maneja_lote)
	{
		$sql="INSERT INTO items (nombre,id_empresa,id_marca,id_categoria,id_medida_sal,id_medida_ing,factor,precio_compra,maneja_lote,id_usuario_crea)
		VALUES ('$nombre','$id_empresa','$id_marca','$id_categoria','$id_ums','$id_umi','$factor','$precio_compra','$maneja_lote','1')";
		//echo $sql;
		return ejecutarConsulta($sql);
	}
	//Implementamos un método para editar registros
	public function editar($id_item,$nombre,$id_empresa,$id_marca,$id_categoria,$id_ums,$id_umi,$factor,$precio_compra,$maneja_lote)
	{

		$sql="UPDATE items SET nombre='$nombre',id_empresa='$id_empresa',id_marca='$id_marca',id_categoria='$id_categoria',id_medida_sal='$id_ums',id_medida_ing='$id_umi',factor='$factor',precio_compra='$precio_compra',maneja_lote='$maneja_lote',id_usuario_modifica='1' WHERE id='$id_item'";

		return ejecutarConsulta($sql);
	}

	//Implementamos un método para desactivar categorías
	public function desactivar($id_ficha)
	{
		$sql="UPDATE ficha_vivienda SET estado=false,fecha_estado=now() WHERE id='$id_ficha'";
		return ejecutarConsulta($sql);
	}

	//Implementamos un método para activar categorías
	public function activar($id_ficha)
	{
		$sql="UPDATE ficha_vivienda SET estado=true,fecha_estado=now() WHERE id='$id_ficha'";
		return ejecutarConsulta($sql);
	}

	

	//Implementar un método para mostrar los datos de un registro a modificar
	public function mostrar($id)
	{
		$sql="select f.id,
		f.id_local  as id_local,
		to_char(f.fecha_inicio,'dd/mm/yyyy')  as fecha_inicio,
		to_char(f.fecha_termino,'dd/mm/yyyy')  as fecha_termino,
		f.id_tipo_actividad,
		f.sector,
		f.id_localidad,u.provincia,u.distrito,
		ta.descripcion as tipo_actividad,
		lo.descripcion as localidad,
		l.nombre   as establecimiento,
		uc.login   as usuario_crea,
		to_char(f.fecha_creacion,'dd/mm/yyyy HH:MI AM')  as fecha_creacion,		
		f.id_tipo_documento_paciente,
    	f.numero_documento_paciente,
    	f.nombres_paciente,
    	f.id_tipo_paciente,
    	f.direccion_ovitrampa,
    	f.id_usuario_inspector

		from  ficha_vivienda f
		inner join locales l on l.id=f.id_local
		left join ubigeo u on u.id=l.id_ubigeo
		inner join tablas ta on ta.id_tipo=f.id_tipo_actividad and ta.id_tabla='18'
		inner join localidades lo on lo.id=f.id_localidad
		left join usuarios uc on uc.id=f.id_usuario_crea 
		where f.id='$id'";


		return ejecutarConsultaSimpleFila($sql);
	}

	public function getItem($id)
	{
		$sql="select d.*,
		(departamento||' - '||provincia||' - '||distrito) as distrito
		from 
		ficha_vivienda_detalle d 
		left join ubigeo u on u.id=d.id_ubigeo
		where d.id='$id'";
		return ejecutarConsulta($sql);
	}

	public function detalleFicha($id)
	{
		$sql="select d.*,cv.descripcion as condicion_vivienda,
				(cnt_tipo_1_i+cnt_tipo_2_i+cnt_tipo_3_i+cnt_tipo_4_i+cnt_tipo_5_i+cnt_tipo_6_i+cnt_tipo_7_i+cnt_tipo_8_i+cnt_tipo_9_i) tot_i,
				(cnt_tipo_1_p+cnt_tipo_2_p+cnt_tipo_3_p+cnt_tipo_4_p+cnt_tipo_5_p+cnt_tipo_6_p+cnt_tipo_7_p+cnt_tipo_8_p+cnt_tipo_9_p) tot_p,
				(cnt_tipo_1_t+cnt_tipo_2_t+cnt_tipo_3_t+cnt_tipo_4_t+cnt_tipo_5_t+cnt_tipo_6_t+cnt_tipo_7_t+cnt_tipo_8_t+cnt_tipo_9_t) tot_t,
				(cnt_tipo_1_v+cnt_tipo_2_v+cnt_tipo_3_v+cnt_tipo_4_v+cnt_tipo_5_v+cnt_tipo_6_v+cnt_tipo_7_v+cnt_tipo_8_v+cnt_tipo_9_v) tot_l,
				f.id_usuario_crea,
				u.distrito,
				(p.ape_paterno||' '||p.ape_materno||' '||p.nombres) as inspector
				from ficha_vivienda_detalle d 
				inner join tablas cv on cv.id_tipo=d.id_condicion_vivienda and cv.id_tabla='19'
				inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
				left join personal p on p.id=d.id_usuario_registro
				left join ubigeo u on u.id=d.id_ubigeo
				where d.id_ficha_vivienda='$id' order by d.id desc";
		return ejecutarConsulta($sql);
	}

	public function contar($sWhere){
		$sql="SELECT count(f.*) AS cnt 
		from  ficha_vivienda f
		inner join locales l on l.id=f.id_local
		inner join tablas ta on ta.id_tipo=f.id_tipo_actividad and ta.id_tabla='18'
		inner join localidades lo on lo.id=f.id_localidad
		left join usuarios uc on uc.id=f.id_usuario_crea
		left join personal p on p.id=f.id_usuario_inspector
		left join ubigeo u on u.id=l.id_ubigeo
		".$sWhere;
		return ejecutarConsultaSimpleFila($sql);
	}
	//Implementar un método para listar los registros
	public function listar($sWhere,$sOrder,$sLimit)
	{
		//$sql="SELECT e.*,u.distrito FROM empresas e
		//LEFT JOIN ubigeo u ON u.id=e.id_ubigeo ".$sWhere." ".$sOrder." ".$sLimit;

		$sql="
		select f.id  as \"f.id\",
		f.id_local  as \"f.id_local\",
		f.fecha_inicio  as \"f.fecha_inicio\",
		f.fecha_termino  as \"f.fecha_termino\",
		f.id_tipo_actividad  as \"f.id_tipo_actividad\",
		f.sector  as \"f.sector\",
		f.id_localidad  as \"f.id_localidad\",
		ta.descripcion as \"ta.descripcion\",
		lo.descripcion as \"lo.descripcion\",
		l.nombre   as \"l.nombre\",
		uc.login   as \"uc.login\",
		f.id_usuario_crea as \"f.id_usuario_crea\",
		f.estado  as \"f.estado\",
		f.nombres_paciente  as \"f.nombres_paciente\",	
		(p.ape_paterno||' '||p.ape_materno||' '||p.nombres) as inspector,	
		to_char(f.fecha_creacion,'dd/mm/yyyy HH:MI AM')  as \"f.fecha_creacion\",
		l.id_ubigeo,
		(u.departamento||' - '||u.provincia||' - '||u.distrito) as distrito
		from  ficha_vivienda f
		inner join locales l on l.id=f.id_local
		inner join tablas ta on ta.id_tipo=f.id_tipo_actividad and ta.id_tabla='18'
		inner join localidades lo on lo.id=f.id_localidad
		left join usuarios uc on uc.id=f.id_usuario_crea
		left join personal p on p.id=f.id_usuario_inspector
		left join ubigeo u on u.id=l.id_ubigeo
		".$sWhere." ".$sOrder." ".$sLimit;
		//return  $sql;
		//echo $sql;
		return ejecutarConsulta($sql);
	}
	//Implementar un método para listar los registros y mostrar en el select
	public function select()
	{
		$sql="SELECT id,nombre FROM empresas ORDER BY nombre";
		return ejecutarConsulta($sql);
	}

	public function registroCompras($desde,$hasta,$id_empresa,$id_proveedor)
	{

		$cri_emp="";
		if ($id_empresa!=''){
			$cri_emp="AND lo.id_empresa IN (";
			$aEmpresas=explode(",", $id_empresa);
			for ($i=0;$i<count($aEmpresas);$i++){
				$cri_emp.=$aEmpresas[$i].",";
			}
			$cri_emp=substr($cri_emp,0,strlen($cri_emp)-1).") ";
		}

		$cri_prov="";
		if ($id_proveedor!='*'){
			$cri_prov="AND ig.id_proveedor IN (";
			$aProv=explode(",", $id_proveedor);
			for ($i=0;$i<count($aProv);$i++){
				$cri_prov.=$aProv[$i].",";
			}
			$cri_prov=substr($cri_prov,0,strlen($cri_prov)-1).") ";
		}


		$sql="SELECT 
		ig.serie_documento,
		ig.numero_documento,
		pro.razon_social,
		td.descripcion AS tipo_documento,
		to_char(ig.fecha,'dd/mm/YYYY') AS fecha,
		i.id_ingreso,
		e.nombre as empresa,
		lo.nombre as local,
		lo.id_empresa,
		i.id_item,
		cat.nombre as item,
		i.factor,
		it.maneja_lote,
		i.factor,
		i.cantidad,
		i.costo_unitario_umc,
		l.numero_lote,
		l.fecha_vencimiento,
		c.descripcion AS categoria,
		m.descripcion AS marca,
		mon.descripcion AS  moneda,
		umi.descripcion AS unidad_medida_ingreso,
		i.id_lote
		FROM
		ingresos_detalle i
		INNER JOIN ingresos ig ON ig.id=i.id_ingreso
		INNER JOIN locales lo ON lo.id=ig.id_local
		INNER JOIN empresas e ON lo.id_empresa=e.id
		INNER JOIN items it ON i.id_item=it.id
		INNER JOIN catalogo cat ON cat.id=it.id_catalogo
		INNER JOIN proveedores pro ON ig.id_proveedor=pro.id
		LEFT JOIN tablas mon ON ig.id_moneda=mon.id_tipo AND mon.id_tabla='14'
		LEFT JOIN lotes l ON i.id_lote = l.id
		LEFT JOIN tablas c ON cat.id_grupo=c.id_tipo AND c.id_tabla='4'
		LEFT JOIN tablas m ON it.id_marca=m.id_tipo AND m.id_tabla='5'
		LEFT JOIN tablas td ON ig.id_tipo_documento=td.id_tipo AND td.id_tabla='2'
		LEFT JOIN tablas umi ON it.id_medida_ing=umi.id_tipo AND umi.id_tabla='6' WHERE ig.fecha>='".$desde." 12:00 AM' 
		AND ig.fecha<='".$hasta." 11:59 PM' ".$cri_emp.$cri_prov."
		ORDER BY e.nombre,ig.fecha,razon_social,td.descripcion,serie_documento,numero_documento";
		return ejecutarConsulta($sql);
	}

	public function detalleIngreso($id_ingreso)
	{
		$sql="SELECT i.id_ingreso,
		lo.id_empresa,
		i.id_item,
		cat.nombre,
		i.factor,
		it.maneja_lote,
		i.factor,
		i.cantidad,
		i.costo_unitario_umc,
		l.numero_lote,
		l.fecha_vencimiento,
		c.descripcion AS categoria,
		m.descripcion AS marca,
		umi.descripcion AS unidad_medida_ingreso,
		i.id_lote
		FROM
		ingresos_detalle i
		INNER JOIN ingresos ig ON ig.id=i.id_ingreso
		INNER JOIN locales lo ON lo.id=ig.id_local
		INNER JOIN items it ON i.id_item=it.id
		INNER JOIN catalogo cat ON cat.id=it.id_catalogo
		LEFT JOIN lotes l ON i.id_lote = l.id
		LEFT JOIN tablas c ON cat.id_grupo=c.id_tipo AND c.id_tabla='4'
		LEFT JOIN tablas m ON it.id_marca=m.id_tipo AND m.id_tabla='5'
		LEFT JOIN tablas umi ON it.id_medida_ing=umi.id_tipo AND umi.id_tabla='6' WHERE i.id_ingreso='$id_ingreso'";
		return ejecutarConsulta($sql);
	}
}

?>
