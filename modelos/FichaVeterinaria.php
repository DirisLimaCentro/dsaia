...
order by 1,2    ";
	...
	public function eliminaItem($id){
		$sql="delete from ficha_veterinaria_detalle where id='$id'";
		return ejecutarConsulta($sql);
	}
	...
	public function save(
		...
		$sql="SELECT sp_crud_header_ficha_veterinaria(
			'$id_ficha',
			'$id_persona',
			'$id_local',
			'$resolucion_numero',
	...
	public function saveFichaDetalle($id_ficha,$id_ficha_detalle,
		...
		$id_ubigeo_propietario
		
	){


		$sql="SELECT sp_crud_body_ficha_veterinaria('$id_ficha','$id_ficha_detalle',
		'$nombre_mascota',
		'$id_especie',
		'$id_sexo',
		'$edad_mascota',
	...
	{
		$sql="UPDATE ficha_veterinaria SET estado=false,fecha_estado=now() WHERE id='$id_ficha'";
		return ejecutarConsulta($sql);
	}
	...
	{
		$sql="UPDATE ficha_veterinaria SET estado=true,fecha_estado=now() WHERE id='$id_ficha'";
		return ejecutarConsulta($sql);
	}
	...
	public function mostrar($id)
	{
		...
		e.nombre as empresa,
		e.nombre_comercial,
		e.ruc,e.e_mail as correo_empresa,
		e.telefono_fijo as telefono_empresa,
		e.direccion as direccion_empresa,
		m.descripcion as name_mes,
		(ue.departamento||' - '||ue.provincia||' - '||ue.distrito) as ubigeo_empresa,		
		(up.departamento||' - '||up.provincia||' - '||up.distrito) as ubigeo_persona,
		(uf.departamento||' - '||uf.provincia||' - '||uf.distrito) as ubigeo_ficha,
		p.nombres,
		p.ape_materno,
		p.ape_paterno,
		p.id_tipo_documento,
		...
		p.telefono,
		p.cmvp,	
		p.id_ubigeo as id_ubigeo_persona,	
		uc.login   as usuario_crea,		
		to_char(f.fecha_creacion,'dd/mm/yyyy HH:MI AM')  as fecha_create
		from  ficha_veterinaria f
		inner join locales l on l.id=f.id_local
		inner join empresas e on e.id=l.id_empresa
		inner join persona p on p.id=f.id_persona
		left join ubigeo ue on ue.id=e.id_ubigeo
		left join ubigeo up on up.id=p.id_ubigeo
		left join ubigeo uf on uf.id=f.id_ubigeo
		left join usuarios uc on uc.id=f.id_usuario_crea 
		left join tablas m on m.id_tipo=f.mes_reporte::varchar and m.id_tabla='66'
		where f.id='$id'";
	...
	{
		$sql="select d.*,
		(u.departamento||' - '||u.provincia||' - '||u.distrito) as ubigeo_propietario
		 from ficha_veterinaria_detalle d 
	...
	public function detalleFicha($id)
	{
		$sql="select 
				...
				(u.departamento||' - '||u.provincia||' - '||u.distrito) as ubigeo_propietario
				from ficha_veterinaria_detalle d
				inner join ficha_veterinaria f on f.id=d.id_ficha_veterinaria
				left join tablas e on e.id_tipo=d.id_especie and e.id_tabla='40'
				left join tablas s on s.id_tipo=d.sexo_mascota and s.id_tabla='82'
				left join ubigeo u on u.id=d.id_ubigeo
				where d.id_ficha_veterinaria='$id' order by d.id desc";
		return ejecutarConsulta($sql);
	}
	...
	public function contar($sWhere){
		...
		inner join locales l on l.id=f.id_local		
		inner join empresas e on e.id=l.id_empresa
		inner join persona per on per.id=f.id_persona
		left join usuarios uc on uc.id=f.id_usuario_crea
	...
	public function listar($sWhere,$sOrder,$sLimit)
	{
		...
		f.resolucion_numero  as \"f.resolucion_numero\",
		f.resolucion_anio  as \"f.resolucion_anio\",
		(per.ape_paterno||' '||per.ape_materno||' '||per.nombres) as medico,
		e.nombre  as \"e.nombre\",	
		e.ruc  as \"e.ruc\",
		(f.resolucion_numero||'-'||f.resolucion_anio) as resolucion,		
