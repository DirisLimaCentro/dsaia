...
Class Descargo
{
	...
	public function actualizar($id_descargo,$id_usuario,$fecha_descargo,$observaciones,$detalle){
		$sql="SELECT sp_update_descargo('$id_descargo','$id_usuario','$fecha_descargo','$observaciones','$detalle')";
		return ejecutarConsulta($sql);
	}

	public function insertar($id_empresa,$id_local,$observaciones,$id_usuario_crea,$id_solicitante,$fecha_descargo,
		$detalle){

		$sql="SELECT sp_create_descargo('$id_empresa','$id_local','$observaciones',
			'$id_usuario_crea','$id_solicitante','$fecha_descargo','$detalle')";	
	...
	{
		$sql="UPDATE descargos SET estado='0',id_usuario_anula='$id_usuario',fecha_anulacion=now() WHERE id='$id_requerimiento'";
		return ejecutarConsulta($sql);
	}
	...
	{
		$sql="UPDATE descargos SET estado='1' WHERE id='$id_requerimiento'";
		return ejecutarConsulta($sql);
	}
	...
	public function contar($sWhere){
		...
		LEFT JOIN personal ps ON ps.id=i.id_responsable ".$sWhere;
		return ejecutarConsultaSimpleFila($sql);
	}
	...
	public function listar($sWhere,$sOrder,$sLimit)
	{
		...
		(ps.ape_paterno||' '||ps.ape_materno||' '||ps.nombres) as personal,
		...
		INNER JOIN empresas e ON l.id_empresa=e.id		
		LEFT JOIN usuarios uc ON uc.id=i.id_usuario_crea
		LEFT JOIN personal ps ON ps.id=i.id_responsable
	...
	public function registroDescargos($desde,$hasta,$id_empresa,$id_local,$id_catalogo)
	{
		...
		$cri_per="";
		/*if ($id_personal!='*'){
			$cri_loc="AND ig.id_encargado_traslado IN (";
			$aProv=explode(",", $id_personal);
			for ($i=0;$i<count($aProv);$i++){
				$cri_per.=$aProv[$i].",";
			}
			$cri_per=substr($cri_per,0,strlen($cri_per)-1).") ";
		}*/

		$cri_cat="";
		if ($id_catalogo!='*'){
			$cri_loc="AND it.id_catalogo IN (";
			$aProv=explode(",", $id_catalogo);
			for ($i=0;$i<count($aProv);$i++){
				$cri_cat.=$aProv[$i].",";
			}
		...
		$sql="SELECT 
		ig.numero,
		(per.ape_paterno||' '||per.ape_materno||' '||per.nombres) as encargado,
		ig.observaciones,
		...
		FROM
		descargos_detalle i
		INNER JOIN descargos ig ON ig.id=i.id_descargo
		...
		INNER JOIN personal per ON ig.id_responsable=per.id		
		...
		AND ig.fecha<='".$hasta." 11:59 PM' ".$cri_emp.$cri_loc.$cri_per.$cri_cat."
		ORDER BY e.nombre,ig.fecha,ig.numero";
		return ejecutarConsulta($sql);
	}

	public function detalleDescargo($id_requerimiento)
	{
		$sql="SELECT i.id_descargo,		
		i.id_item,
		cat.nombre,
		i.cantidad,		
		c.descripcion AS categoria,
		...
		FROM
		descargos_detalle i	
		INNER JOIN items it on it.id=i.id_item	
		INNER JOIN catalogo cat ON it.id_catalogo=cat.id
		LEFT JOIN tablas c ON cat.id_grupo=c.id_tipo AND c.id_tabla='4'
		LEFT JOIN tablas m ON it.id_marca=m.id_tipo AND m.id_tabla='5'
		LEFT JOIN tablas umi ON it.id_medida_ing=umi.id_tipo AND umi.id_tabla='6' WHERE i.id_descargo='$id_requerimiento'";
		return ejecutarConsulta($sql);
	}
