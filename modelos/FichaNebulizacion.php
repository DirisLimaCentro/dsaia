...
Class Ficha
{
	...
	public function getCoordenadasFumigacion($desde,$id_establecimiento)
	{
		$cri="";
		...
		$sql="select * from ficha_rutas_fumigacion where fecha_programada='$desde' ".$cri;
	...
	public function getCoordenadas($desde,$hasta,$tipo,$id_establecimiento)
	{
		...
		if ($tipo=='C'){
			...
			where 
			fv.fecha_inicio>='$desde' and fv.fecha_termino<='$hasta' and 
			fvd.id_condicion_vivienda in ('6') and 
			(latitud is not null and latitud<>'' and longitud<>'') 			
			".$cri." order by fvd.id";	
		}
	...
	public function dataFumigacion($desde,$tipo,$id_establecimiento){
		...
		where fecha_programada='$desde' ".$cri;

		return ejecutarConsulta($sql);
	}

	public function positivosDistrito($desde,$hasta,$tipo,$id_establecimiento)
	{
		/*$sql="select 
		u.distrito,
	...
	public function reportAedico($mes,$anio,$tipo,$nivel,$id_local){
		...
		/*$sql="select ris,sum(inspeccionados) as inspeccionados,sum(positivos) as positivos from (
			...
			inner join ficha_vivienda f on f.id=d.id_ficha_vivienda
			inner join locales l on l.id=f.id_local
			where f.estado=true and date_part('month',fecha_inicio)='3'
			and date_part('year',fecha_inicio)='2024'
			and id_condicion_vivienda='6'
			group by l.nombre
			)x group by ris 
			union all
...
order by 1,2    ";
	...
	public function resumenAedesDetalle($mes,$anio,$id_establecimiento){
		...
		left join personal per on per.id=fd.id_usuario_registro
		where f.estado=true and date_part('year',fecha_inicio)='".$anio."' and 
		date_part('month',fecha_inicio)='".$mes."'".$cri_cat."
	...
	public function resumenAedes($mes,$anio,$id_establecimiento){
		...
		inner join ubigeo u on u.id=l.id_ubigeo
		inner join tablas tc on tc.id_tipo=fd.id_condicion_vivienda and tc.id_tabla=''19''
		where f.estado=true and date_part(''year'',fecha_inicio)=".$anio." and date_part(''month'',fecha_inicio)=".$mes.$cri_cat." 
	...
	public function eliminaItem($id){
		$sql="delete from ficha_nebulizacion_detalle where id='$id'";
		return ejecutarConsulta($sql);
	}
	...
	public function save(
		...
		$sql="SELECT 
		sp_crud_header_ficha_nebulizacion(
		'$id_ficha',
		'$id_local_destino',
		'$sector_neb',
		'$fecha_neb',	
	...
	public function saveFichaDetalle(
		$id_ficha,
		$id_ficha_detalle,		
		$direccion,
		$latitud,
		...
		$nro_pisos,
		$nro_residentes,
		$id_condicion_vivienda,
		$mezcla,
		$gasolina,
		$id_usuario
		){

		$sql="SELECT sp_crud_body_ficha_nebulizacion(
		'$id_ficha',
		'$id_ficha_detalle',		
		'$direccion',
		'$latitud',
	...
	public function mostrar($id)
	{
		...
		f.id_local ,
		l.nombre as establecimiento,
		to_char(f.hora_inicio,'hh:mi') as hora_inicio ,
		to_char(f.hora_termino,'hh:mi')  as hora_termino,
		f.sector ,
		f.vuelta ,
		f.nro_brigada ,
		f.tipo_maquina ,
		...
		f.id_nebulizador,
		f.id_jefe_brigada,
		(j.ape_paterno||' '||j.ape_paterno||' '||j.nombres) as jefe_brigada,
		(n.ape_paterno||' '||n.ape_paterno||' '||n.nombres) as nebulizador,
		f.id_usuario_crea ,
		f.fecha_creacion ,
		uc.login 
		from ficha_nebulizacion f
		inner join locales l on l.id=f.id_local
		left join tablas t on t.id_tipo=f.id_turno and t.id_tabla='89' 
		left join personal j on j.id=f.id_jefe_brigada
		left join personal n on n.id=f.id_nebulizador
	...
	{
		$sql="select * from ficha_nebulizacion_detalle where id='$id'";
		return ejecutarConsulta($sql);
	}
	...
	{
		$sql="select d.*,
		c.descripcion as condicion_vivienda
		from 
		ficha_nebulizacion_detalle d
		left join tablas c on c.id_tipo=d.id_condicion_vivienda and c.id_tabla='90'
		where id_nebulizacion='$id'
		order by id desc";
		return ejecutarConsulta($sql);
	}
	...
	public function listar($sWhere,$sOrder,$sLimit)
	{
		...
		f.hora_inicio as \"f.hora_inicio\",
		f.hora_termino as \"f.hora_termino\",
		f.sector as \"f.sector\",
		f.vuelta as \"f.vuelta\",

		f.nro_brigada as \"f.nro_brigada\",

		f.tipo_maquina as \"f.tipo_maquina\",
		t.descripcion as \"t.descripcion\",

		f.insecticida as \"f.insecticida\",
		(j.ape_paterno||' '||j.ape_paterno||' '||j.nombres) as jefe_brigada,
		f.id_usuario_crea as \"f.id_usuario_crea\",		

		f.fecha_creacion as \"f.fecha_creacion\",
