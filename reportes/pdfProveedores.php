...
class PDF extends PDF_MC_Table
{
	...
	function Header()
	{
		...
		$this->Cell(100,5,'NOMBRE - RAZON SOCIAL','LT',0,'C',true);
		$this->Cell(20,5,'RUC','LT',0,'C',true);		
		$this->Cell(90,5,'DIRECCION','LT',0,'C',true);
		$this->Cell(20,5,'TELEFONO','LT',0,'C',true);
		$this->Cell(30,5,'CELULAR','LTBR',0,'C',true);
		$this->Cell(30,5,'EMAIL','LTBR',1,'C',true);
...
$pdf->SetWidths(array(100,20,90,20,30,30));
$pdf->SetAligns(array('L','L','L','L','L','L'));

$tot=0;
while ($reg=pg_fetch_object($rs)){
	$pdf->Row(array(utf8_decode($reg->{'razon_social'}),$reg->{'ruc'},utf8_decode($reg->{'direccion'}.'-'.$reg->{'distrito'}),
	$reg->{'telefono_fijo'},$reg->{'celular'},$reg->{'e_mail'}));
}	
