...
$oc=$descargo->mostrar($_GET["id"]);
 		
$rs=$descargo->detalleDescargo($_GET["id"]);
...
$pdf->Cell(20,6,$oc->numero,'LTBR',1,'R');
$pdf->SetFont('Arial','',8);
...
$pdf->Cell(30,6,'Usuario','LB',0,'L');
$pdf->Cell(102,6,': '.$oc->usuario_crea,'RB',1,'L');
...
$pdf->Cell(105,6,'Descripcion','LT',0,'C',true);
$pdf->Cell(45,6,'Marca','LT',0,'C',true);
...
$pdf->SetWidths(array(10,105,45,15,25));
...
while ($reg=pg_fetch_object($rs)){
	//$tot+=$reg->costo_total;
	$corr++;
	$pdf->Row(array($corr,utf8_decode($reg->nombre),utf8_decode($reg->marca),$reg->cantidad,$reg->unidad_medida_ingreso));
}	
