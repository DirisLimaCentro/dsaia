...
class PDF extends PDF_MC_Table
{
	...
	function Header()
	{
		...
		$this->Cell(30,5,'PROVEEDOR','LTBR',0,'C',true);
		$this->Cell(60,5,'ITEM','LTBR',0,'C',true);
		$this->Cell(20,5,'PRECIO','LTBR',0,'C',true);
		$this->Cell(20,5,'CNT','LTBR',0,'C',true);
...
while ($reg=pg_fetch_object($rs)){
	...
	$reg->{'serie_documento'},$reg->{'numero_documento'},utf8_decode($reg->{'razon_social'}),($reg->{'item'}.'-'.$reg->{'unidad_medida_ingreso'}.'-'.$reg->{'marca'}),$reg->{'costo_unitario_umc'},
	$reg->{'cantidad'},round(($reg->{'cantidad'}*$reg->{'costo_unitario_umc'}),2)
	));
	$tot+=round(($reg->{'cantidad'}*$reg->{'costo_unitario_umc'}),2);
}
