...
{
	*/
require_once "../modelos/FichaSanAlimentacion.php";
...
switch ($_GET["op"]){
	case 'save':
		$rs=$ficha->save(
			...
			$_POST['id_local'], 
			$_POST['id_usuario'],
			$_POST['id_medico_jefe'],
			$_POST['fecha'],
			limpiarCadena($_POST['responsable_modulo_venta']),
			limpiarCadena($_POST['numero_carnet']),
			$_POST['numero_manipuladores'],			
			...
			$_POST['iii_4'],
			$_POST['iii_5'],
			$_POST['cnt_extintores'],
			$_POST['cnt_senalizadores'],
			$_POST['cnt_botiquin'],
			$_POST['cnt_sep']
	...
	case 'listar':
		...
		$aColumns = array( 'f.id', 'l.nombre', 'f.id', 'f.fecha','f.responsable_modulo_venta','f.numero_carnet',
			'f.numero_manipuladores','uc.login','f.fecha_creacion');
		/* Indexed column (used for fast and accurate table cardinality) */
    	$sIndexColumn = "f.id";
    	$sOrder="";
		...
		$cnt = $a->cnt;
 		...
 		while ($reg=pg_fetch_object($rspta)){
 			...
 			$data[]=array( 				
 				...
 				"2"=>$reg->{'f.id'},
 				"3"=>$reg->{'f.fecha'},
 				"4"=>$reg->{'f.responsable_modulo_venta'},
 				"5"=>$reg->{'f.numero_carnet'},
 				"6"=>$reg->{'f.numero_manipuladores'},
 				"7"=>$reg->{'uc.login'},
