...
switch ($_GET["op"]){
	...
	case 'list':
		...
		$data=array();
		while ($reg=pg_fetch_object($rs)){
 			$data[]=array("id"=>$reg->id, "nombre"=>$reg->persona);
 		}
	...
	case 'guardaryeditar':
		if (empty($id_personal)){
			...
			$rspta=$personal->insertar($nombre,$tipo_documento,$numero_documento,$sexo,$nombre,$apellido_paterno,$apellido_materno,$ubigeo,$direccion,$telefono,$celular,$email,$_POST["autoriza_orden_compra"],$_POST["autoriza_requerimiento"],$_POST["externo"],$_POST["id_condicion_laboral"]);

			echo $rspta ? "Personal registrado" : "Personal no se pudo registrar";
		}
		else {
			$rspta=$personal->editar($id_personal,$nombre,$tipo_documento,$numero_documento,$sexo,$nombre,$apellido_paterno,$apellido_materno,$ubigeo,$direccion,$telefono,$celular,$email,$_POST["autoriza_orden_compra"],$_POST["autoriza_requerimiento"],$_POST["externo"],$_POST["id_condicion_laboral"]);
	...
	case 'listarp':
		...
		while ($reg=pg_fetch_object($rspta)){
			$json[] = array('id' => $reg->id, 'text' => $reg->persona, 'nombres'=>$reg->nombres, 'ape_paterno'=>$reg->ape_paterno);
		}
	...
	case 'listar':
		...
		$aColumns = array( 'id','id','ape_paterno','ape_materno','nombres','nro_documento','celular','e_mail','u.distrito','direccion' );
