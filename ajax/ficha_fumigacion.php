...
{
	*/
require_once "../modelos/FichaFumigacion.php";
...
switch ($_GET["op"]){
	...
	case 'saveFicha':
		...
		$rs=$ficha->save(
			...
			$_POST['hora_salida'],
			$_POST['hora_llegada'],
			limpiarCadena($_POST['area']),
			limpiarCadena($_POST['participa']),
			limpiarCadena($_POST['organiza']),
			limpiarCadena($_POST['resumen']),
			limpiarCadena($_POST['detail']),
			$_POST['id_usuario']
		);
		$rpta=pg_fetch_array($rs);
		echo $rpta[0];
	...
	case 'saveFichaDetalle':
		$rs=$ficha->saveFichaDetalle(
			...
			limpiarCadena($_POST['lugar']),
			limpiarCadena($_POST['direccion_ruta']),
			$_POST['latitud_ruta'],
			$_POST['longitud_ruta'],
			$_POST['orden'],
			$_POST['id_usuario']
		...
		$cnt = $a->cnt;
 		...
 		while ($reg=pg_fetch_object($rspta)){
 			...
 			$data[]=array(
 				...
 				"2"=>$reg->{'f.fecha_programada'},
 				"3"=>$reg->{'f.hora_salida'},
 				"4"=>$reg->{'l.nombre'},
 				"5"=>$reg->{'f.hora_llegada'},
 				"6"=>$reg->{'f.area'},
 				"7"=>$reg->{'f.participa'},
 				"8"=>$reg->{'f.organiza'},
 				"9"=>$reg->{'f.actividad'},
