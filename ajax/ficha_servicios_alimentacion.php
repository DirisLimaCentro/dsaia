...
{
	*/
require_once "../modelos/FichaServAlimentacion.php";
...
switch ($_GET["op"]){
	case 'save':
		$rs=$ficha->save(
			...
			$_POST['id_ubigeo'],
			$_POST['fecha'],
			limpiarCadena($_POST['nombre_servicio']),
			limpiarCadena($_POST['direccion']),
			$_POST['numero_manipuladores'],			
			limpiarCadena($_POST['ubicacion_empresa']),	
			limpiarCadena($_POST['responsable_servicio']),
			limpiarCadena($_POST['id_forma_servicio']),
	...
	case 'listar':
		...
		$aColumns = array( 'f.id', 'l.nombre', 'f.id', 'f.fecha','f.nombre_servicio','fs.descripcion',
		...
		$cnt = $a->cnt;
 		...
 		while ($reg=pg_fetch_object($rspta)){
 			...
 			$data[]=array( 				
 				...
 				"2"=>$reg->{'f.id'},
 				"3"=>$reg->{'f.fecha'},
 				"4"=>$reg->{'f.nombre_servicio'},
 				"5"=>$reg->{'fs.descripcion'},
