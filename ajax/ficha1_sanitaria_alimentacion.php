...
{
	*/
require_once "../modelos/Ficha1_SanAlimentacion.php";
...
switch ($_GET["op"]) {
	...
	case 'listar':
		...
		while ($reg = pg_fetch_object($rspta)) {
			...
			// boton de impresion para hacer reporte en pdf 
			$btnPrint = "<a target='_blank' href='../reportes/formato_1_pdf.php?id=" . $reg->{'f.id'} . "  'print_compra'   ><i class='fa fa-print text-warning'></i></a>";


			/* 'print_compra(".$reg->{'f.id'}.")' */
			$data[] = array(
