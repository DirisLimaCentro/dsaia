...
switch ($_GET["op"]){
	...
	case 'saveFicha':
		...
		$rs=$ficha->save(
			...
			$_POST['cmvp_persona'],
			$_POST['mes_reporte'],
			$_POST['vac_ant_pri_men_1'],
			$_POST['vac_ant_pri_may_1'],
			$_POST['vac_ant_rev_men_1'],
			$_POST['vac_ant_rev_may_1'],
			$_POST['vac_sex_pri_men_1'],
			$_POST['vac_sex_pri_may_1'],
			$_POST['vac_sex_rev_men_1'],
			$_POST['vac_sex_rev_may_1'],
			$_POST['ubigeo_reporte'],
			$_POST['id_usuario']);
		$rpta=pg_fetch_array($rs);
		echo $rpta[0];
	break;
	...
	case 'saveFichaDetalle':
		$rs=$ficha->saveFichaDetalle(
			...
			limpiarCadena($_POST['nombre_mascota']),
			$_POST['id_especie'],
			$_POST['id_sexo'],
			limpiarCadena($_POST['edad_mascota']),
			limpiarCadena($_POST['propietario']),
			$_POST['telefono_propietario'],
			$_POST['id_ubigeo_propietario']
 						...
 						<table id="dt<?=$_GET['id']; ?>" class="table table-striped table-bordered table-hover " >
 							<thead >
 								<tr class="v-grid-header">
 									...
 									<th>Sexo</th>
 									<th>Edad</th>
 									<th>Nombre Apellido Propietario</th>
 							...
 							<tbody>
 								...
 								while ($reg=pg_fetch_object($rs)){
 									...
 									echo "<td >".$reg->nombre_mascota."</td>";
 									echo "<td >".$reg->sexo."</td>";
									echo "<td >".$reg->edad_mascota."</td>";
									echo "<td >".$reg->propietario."</td>";
									echo "<td >".$reg->telefono."</td>";									
									echo "<td >".$reg->ubigeo_propietario."</td>";
	...
	case 'listar':
		...
		$aColumns = array( 'f.id','f.id', 'e.nombre','f.id', 'e.ruc','resolucion','medico','uc.login','f.fecha_creacion' );
		...
		$cnt = $a->cnt;
 		...
 		while ($reg=pg_fetch_object($rspta)){
 			...
 			$url='../reportes/rptConsolidadoVacunas.php?id='.$reg->{'f.id'};
			$btnPrint="<a target='_blank' href='".$url."'><i class='fa fa-print text-success'></i></a>";
