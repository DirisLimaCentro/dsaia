...
switch ($_GET["op"]){
	...
	break;
	case 'saveCompra':
		$rs=$compra->insertar($_POST['id_local'],$_POST['id_tipo_documento'],$_POST['serie_documento'],$_POST['numero_documento'],$_POST['fecha_compra'],$_POST['id_moneda'],$_POST['id_forma_pago'],$_POST['tipo_cambio'],$_POST['porcentaje_igv'],$_POST['numero_guia'],$_POST['id_proveedor'],$_POST['observacion'],$_POST['id_tipo_ingreso'],to_pg_array($aDet),'','1',$_POST['id_orden_compra']);
		$rpta=pg_fetch_array($rs);
		echo $rpta[0];
	break;
	case 'detalleCompra':
	...
	case 'listar':
		...
		$aColumns = array( 'i.id','i.id', 'e.nombre', 'l.nombre', 'td.descripcion','i.serie_documento','i.numero_documento','i.fecha','mc.descripcion','pro.razon_social','fp.descripcion' );
		...
		if ($sWhere==''){
	    	$sWhere=" WHERE id_tipo_ingreso='1' ";
	    }else{
	    	$sWhere.=" AND id_tipo_ingreso='1' ";
	    }
		...
		$rspta=$compra->listar($sWhere,$sOrder,$sLimit);
		...
		$cnt = $a->cnt;
 		...
 		while ($reg=pg_fetch_object($rspta)){
 			...
 			$data[]=array(
 				...
 				"9"=>$reg->{'pro.razon_social'},
 				"10"=>$reg->{'fp.descripcion'},
 				"11"=>$stat ,
 				"12"=>$reg->{'i.estado'}
 				);
 		}
