...
{
	*/
require_once "../modelos/FichaAtencionRabia.php";
...
$aDet=array();
if (isset($_POST['id_localizacion'])){
	$aesp=explode(",", $_POST['id_localizacion']);
...
switch ($_GET["op"]){
	case 'save':  //55
		$rs=$ficha->save(
			...
			$_POST['ficha_persona'],
			$_POST['ficha_animal'],
			$_POST['historia_clinica'],
			$_POST['id_especie'],
			$_POST['id_raza'],
			$_POST['fecha_atencion'],
			$_POST['fecha_accidente'],
			$_POST['ubigeo_accidente'],
			$_POST['id_estado_actual_animal'],
			$_POST['tipo_exposicion'],
			$_POST['tipo_doc'],
			$_POST['numero_doc'],
			...
			limpiarCadena($_POST['direccion_referencia']),
			$_POST['telefono'],
			$_POST['e_mail'],
			$_POST['id_grado_instruccion'],
			$_POST['tipo_doc_acompanante'],
			$_POST['numero_doc_acompanante'],
			limpiarCadena($_POST['nombres_acompanante']),
			limpiarCadena($_POST['ape_paterno_acompanante']),
			limpiarCadena($_POST['ape_materno_acompanante']),
			$_POST['mordedura'],
			$_POST['aranazo'],
			$_POST['contacto'],
			$_POST['superficial'],
			$_POST['profunda'],
			to_pg_array($aDet),
			$_POST['antecedentes_vacunacion'],
			$_POST['enfermedad_actual'],			
			limpiarCadena($_POST['otra_localizacion']),
			$_POST['id_tipo_proteccion'],
			$_POST['id_numero_lesiones'],
			$_POST['id_estado_herida'],
			$_POST['id_atencion_herida'],
			$_POST['id_lugar_suceso'],
			$_POST['fecha_vacunacion'],
			$_POST['nro_dosis'],
			$_POST['id_alergico'],			
			limpiarCadena($_POST['descripcion_enfermedad']),
			$_POST['id_tipo_propietario'],
			limpiarCadena($_POST['descripcion_otros']),
			$_POST['id_situacion'],						
			$_POST['id_local'],
			$_POST['id_usuario']
			);
	...
	case 'listar':
		...
		$aColumns = array( 'f.id', 'l.nombre', 'f.id', 'f.fecha_atencion','f.fecha_accidente','ua.distrito',
			'persona','uc.login','f.fecha_creacion');
		/* Indexed column (used for fast and accurate table cardinality) */
    	$sIndexColumn = "f.id";
    	$sOrder="";
		...
		$cnt = $a->cnt;
 		...
 		while ($reg=pg_fetch_object($rspta)){
 			...
 			$data[]=array( 				
 				...
 				"1"=>$reg->{'l.nombre'},
 				"2"=>$reg->{'f.id'},
 				"3"=>$reg->{'f.fecha_atencion'},
 				"4"=>$reg->{'f.fecha_accidente'},
 				"5"=>$reg->{'ua.distrito'},
 				"6"=>$reg->{'persona'},
 				"7"=>$reg->{'uc.login'},
