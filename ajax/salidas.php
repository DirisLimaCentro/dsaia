...
switch ($_GET["op"]){
	...
	<div class="" id="div_detalleLocal">
		<div class="row">
            ...
            while ($reg=pg_fetch_object($rs)){
              ...
              $cnt_items++;
							$btn='<button disabled class="btn btn-link btn-xs " onclick="action_show_item('.$_GET['id_salida'].','.$reg->id_item.','.$reg->id_lote.')"><i  class="fa fa-pencil text-success"></i></button><button disabled class="btn btn-link btn-xs" onclick="action_remove_item('.$_GET['id_salida'].','.$reg->id_item.','.$reg->cantidad.')"><i class="fa fa-close text-success"></i></button>';
							$stat='';
							echo "<tr>";
  ...
  break;
	...
	$aColumns = array( 'id','id','e.nombre','li.nombre','td.descripcion', 's.serie_documento', 's.numero_documento','s.fecha_salida',"trim(p.ape_paterno)||' '||trim(p.ape_materno)||' '||trim(p.nombres)",'s.id_requerimiento' );
