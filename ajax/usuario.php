...
*/
	...
	// BUSCA ESTA SECCIÓN (case 'verificar'):
	case 'verificar':

		// ========== SOLO PROCESAR SI ES POST ==========
		if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
			exit; // Silenciosamente no hacer nada si es GET
		}

		// ========== VALIDAR QUE EXISTAN LOS DATOS POST ==========
		if (
			!isset($_POST['logina']) || !isset($_POST['clavea']) ||
			!isset($_POST['id_empresa']) || !isset($_POST['id_local'])
		) {
		...
		// 2. Rate Limiting - Bloquear tras 5 intentos
		$intentos = $_SESSION['login_intentos'] ?? 0;
		$ultimo = $_SESSION['login_tiempo'] ?? 0;

		if ($intentos >= 5 && (time() - $ultimo) < 300) {
			echo json_encode([
				'success' => false,
		...
		if(isset($_POST['captcha'])){
			...
			$captcha_correct = ($_SESSION['captcha_num1'] ?? 0) + ($_SESSION['captcha_num2'] ?? 0);
		...
		if (empty($id_empresa) || empty($id_local)) {
			echo json_encode(['success' => false, 'message' => 'Debe seleccionar empresa y área']);
			exit;
		}
