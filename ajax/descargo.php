...
if (isset($_POST['detalle'])){
	...
	for ($i=0;$i<count($_POST['detalle']);$i++){
		$aDet[$i]=array($_POST['detalle'][$i]['id_item'],$_POST['detalle'][$i]['cantidad']);
...
switch ($_GET["op"]){
	...
	case 'updateDescargo':
		...
		$rs=$descargo->actualizar($_POST['id_requerimiento'],$_POST['id_usuario_crea'],	$_POST['fecha_descargo'],		
			limpiarCadena($_POST['observaciones']),to_pg_array($aDet));
		$rpta=pg_fetch_array($rs);
		echo $rpta[0];
	break;
	case 'saveDescargo':
		$rs=$descargo->insertar($_POST['id_empresa'],$_POST['id_local'],limpiarCadena($_POST['observaciones']),$_POST['id_usuario_crea'],$_POST['id_solicitante'],
			$_POST['fecha_descargo'],to_pg_array($aDet));
		$rpta=pg_fetch_array($rs);
		echo $rpta[0];
	break;
	case 'detalleDescargo':
		$rs=$descargo->detalleDescargo($_GET['id_requerimiento']);
		$data=array();
	...
	case 'desactivar':
		$rspta=$descargo->desactivar($_POST['id_requerimiento'],$_SESSION['s_id_usuario']);
 		echo $rspta ? "Registro anulado" : "Registro no se puede anular";
	break;

	case 'activar':
		$rspta=$descargo->activar($_POST['id_requerimiento'],$_POST['id_usuario']);
	...
	break;
	case 'mostrar':
		$oc=$descargo->mostrar($_POST["id_requerimiento"]); 		
 		$rs=$descargo->detalleDescargo($_POST["id_requerimiento"]);
 		$adet=pg_fetch_all($rs);
		$data=array(
			'fecha'=>$oc->fecha,				
			'numero'=>$oc->numero,
	...
	case 'listar':
		...
		$aColumns = array( 'i.id','i.id', 'e.nombre', 'i.numero','l.nombre', 'i.fecha','uc.login','i.estado' );
		...
		$rspta=$descargo->listar($sWhere,$sOrder,$sLimit);

				
		$a =$descargo->contar($sWhere);
		$cnt = $a->cnt;
 		...
 		while ($reg=pg_fetch_object($rspta)){
 			...
 			/*if ($reg->{'i.fecha_autorizacion'}==''){*/
 				...
 				if ($_SESSION['s_id_usuario']==$reg->{'i.id_usuario_crea'}){
 			...
 			$btnEdit=($reg->{'i.estado'}=='1')?"<li><a href='#' onclick='mostrar_descargo(".$reg->{'i.id'}.",".$_SESSION['s_id_nivel'].")'><i class='fa fa-pencil text-success'></i> Editar</a>
 			</li>":"";

 			$url='../reportes/rptDescargo.php?id='.$reg->{'i.id'};
