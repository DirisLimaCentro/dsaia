...
{
	*/
require_once "../modelos/ficha_acta_condiciones_sanitarias_xl.php";
...
switch ($_GET["op"]){
	case 'save':
		...
		$responsable_establecimiento=$data['responsable_establecimiento'];
		$licencia=$data['licencia'];
		$responsable_calidad=$data['responsable_calidad'];
		$cargo=$data['cargo'];

		$dias_actividad=$data['dias_actividad'];
		$tipo_establecimiento=$data['tipo_establecimiento'];
		$nro_hombres=$data['nro_hombres'];
		$tipo_qaliwarma=$data['tipo_qaliwarma'];
...
//tabla4
            ...
            $data['i_otras_observaciones_obs'],
            $data['i_recomendaciones_obs'],
	...
	case 'listar':
		...
		$aColumns = array( 'f.id', 'l.nombre', 'f.id', 'f.fecha_inicio','e.nombre','e.ruc',
			'responsable_calidad','uc.login','f.fecha_creacion');
		/* Indexed column (used for fast and accurate table cardinality) */
    	$sIndexColumn = "f.id";
    	$sOrder="";
