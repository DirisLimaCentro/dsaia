...
{
	*/
require_once "../modelos/FichaVigilanciaPiscina.php";
...
switch ($_GET["op"]) {
	...
	case 'reportControlPiscinasxEstablecimiento':
		$rs = $ficha->reportControlPiscinasxEstablecimiento($_POST['anio'], $_POST['mes']);
		$dat = array();
		while ($reg = pg_fetch_object($rs)) {
			$dat[] = $reg;
		}

		$rs = $ficha->reportControlPiscinasxEstablecimientoEntidad($_POST['anio'], $_POST['mes']);
		$datdrill = array();
		while ($reg = pg_fetch_object($rs)) {
			$datdrill[] = $reg;
		}

		$rs = $ficha->reportCondicionxEstablecimiento($_POST['anio'], $_POST['mes']);
		$datcond = array();
		while ($reg = pg_fetch_object($rs)) {
			$datcond[] = $reg;
		}

		$datreport = array('array_data' => $dat, 'array_drill' => $datdrill, 'array_condicion' => $datcond);
		echo json_encode($datreport);
		break;

	case 'reportControlPiscinasxDistrito':
		$rs = $ficha->reportControlPiscinasxDistrito($_POST['anio'], $_POST['mes']);
		$dat = array();
		while ($reg = pg_fetch_object($rs)) {
			$dat[] = $reg;
		}

		$rs = $ficha->reportControlPiscinasxEntidad($_POST['anio'], $_POST['mes']);
		$datdrill = array();
		while ($reg = pg_fetch_object($rs)) {
			$datdrill[] = $reg;
		}

		$rs = $ficha->reportCondicionxDistrito($_POST['anio'], $_POST['mes']);
		$datcond = array();
		while ($reg = pg_fetch_object($rs)) {
			$datcond[] = $reg;
		}
	...
	case 'save':
		...
		$arr_entidad = array(
			$_POST['ruc'], $_POST['razon_social'], $_POST['ubigeo'], trim($_POST['direccion_empresa']), trim($_POST['telefono_fijo_empresa']), trim($_POST['celular_empresa']), trim($_POST['email_empresa']), trim($_POST['nombre_representante']), $_POST['tipo_doc'], trim($_POST['numero_doc']), trim($_POST['nombres']), trim($_POST['ape_paterno']), trim($_POST['ape_materno']),

			trim($_POST['long_este']),
			trim($_POST['long_norte'])

		);


		$arr_dato = array($_POST['id_local'], $_POST['id_usuario'], $_POST['fecha_inicio_ficha'], trim($_POST['hora_inicio']), trim($_POST['hora_fin']), $_POST['id_motivo_ficha'], $_POST['id_instalacion_lavapies'], $_POST['id_instalacion_sshh'], $_POST['id_estado_sistema_recirculacion'], $_POST['chk_limpieza_local'], $_POST['id_estado_estanque'], $_POST['chk_criador_aedes_aegypti'], $_POST['chk_cuerpos_agua'], $_POST['chk_cuaderno_registro'], $_POST['chk_aprobacion_sanitaria'], $_POST['nro_aprobacion_sanitaria'], '', $_POST['resul_cloro_residual_1'], $_POST['resul_ph_1'], $_POST['resul_turbidez_1'], $_POST['resul_temperatura_1'], $_POST['resul_cloro_residual_2'], $_POST['resul_ph_2'], $_POST['resul_turbidez_2'], $_POST['resul_temperatura_2'], $_POST['resul_cloro_residual_3'], $_POST['resul_ph_3'], $_POST['resul_turbidez_3'], $_POST['resul_temperatura_3'], $_POST['resul_cloro_residual_4'], $_POST['resul_ph_4'], $_POST['resul_turbidez_4'], $_POST['resul_temperatura_4']);
		$arr_detalle = array('');

		//print_r('{'.to_pg_array($arr_dato[0]).'}'); exit();
		$accion = ($_POST['id_ficha'] == '0') ? 'I' : 'A';
		$rs = $ficha->save(
			$accion,
			$_POST['id_ficha'],
			'{' . to_pg_array($arr_entidad) . '}',
			'{' . to_pg_array($arr_dato) . '}',
			'{' . to_pg_array($arr_detalle) . '}',
			$_POST['id_usuario'],
			$_POST['resultado']
		);
...
<?
	...
	case 'listar':
		...
		$aColumns = array(
			'f.id', 'l.nombre', 'f.id', 'f.fecha', 'e.nombre', 'e.ruc',
			'responsable', 'uc.login', 'f.fecha_creacion'
		);
		/* Indexed column (used for fast and accurate table cardinality) */
		$sIndexColumn = "f.id";
		$sOrder = "";
		...
		while ($reg = pg_fetch_object($rspta)) {
			...
			$stat = ($reg->{'resultado'} == 't') ? '<div class="text-center"><span class="label bg-green">Apto</span></div>' : '<div class="text-center"><span class="label bg-red">No Apto</span></div>';
